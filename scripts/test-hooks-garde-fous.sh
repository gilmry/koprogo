#!/usr/bin/env bash
# ── Ce fichier s'appelait `test-guardrail-hooks.sh`, et il est renommé ──────
#
# Deux stories ont créé le même nom de fichier pour deux objets différents :
#
#   #429  →  témoin du modèle d'autorisation Tier 1 / Tier 2
#   #425  →  témoin des garde-fous `.claude/hooks/*.sh`   ← CE FICHIER
#
# Les deux sont légitimes, aucune ne dépendait de l'autre, et le fan-out les
# a produites en parallèle sans qu'aucune ne puisse voir l'autre. Le conflit
# n'est donc pas une divergence : c'est une collision de nommage.
#
# Sacrifier l'une des deux aurait perdu un témoin. On garde les deux, et
# celle-ci prend le nom qui dit ce qu'elle vérifie.
#
