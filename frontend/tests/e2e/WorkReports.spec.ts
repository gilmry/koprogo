...
async function setupSyndicWithBuilding(page: Page): Promise<{
  ...
  const email = `workreport-test-${timestamp}@example.com`;
  ...
  const orgResp = await page.request.post(`${API_BASE}/organizations`, {
    data: {
      name: `WorkReport Test Org ${timestamp}`,
      slug: `wr-test-${timestamp}`,
      contact_email: email,
  ...
  const buildingResp = await page.request.post(`${API_BASE}/buildings`, {
    data: {
      name: `WorkReport Building ${timestamp}`,
      address: `${timestamp} Rue Travaux`,
      city: "Antwerp",
      postal_code: "2000",
      country: "Belgium",
      total_units: 6,
