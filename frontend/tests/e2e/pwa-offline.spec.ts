...
test.describe('PWA Capabilities', () => {
  ...
  test('should show offline status when network is down', async ({ page, context }) => {
    // Login d'abord
    await page.goto('/login');
    await page.fill('input[type="email"]', 'test@test.com');
    await page.fill('input[type="password"]', 'test123');
    ...
    await expect(page.locator('text=Hors ligne')).toBeVisible({ timeout: 10000 });
  ...
  test('should allow manual synchronization', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Sync...")')).toBeVisible({ timeout: 1000 });
