...
function flatten(obj: Record<string, unknown>, prefix = ""): Set<string> {
  ...
  for (const [k, v] of Object.entries(obj)) {
    ...
    if (v !== null && typeof v === "object" && !Array.isArray(v)) {
      for (const nested of flatten(v as Record<string, unknown>, `${key}.`))
        keys.add(nested);
    } else {
      keys.add(key);
    }
...
function sourceFiles(dir: string, acc: string[] = []): string[] {
  for (const entry of readdirSync(dir)) {
    ...
    else if ([".svelte", ".astro", ".ts"].includes(extname(entry)))
      acc.push(full);
  }
  return acc;
}
...
const catalogs = Object.fromEntries(
  LOCALES.map((l) => [
    l,
    flatten(
      JSON.parse(readFileSync(join(SRC, "locales", `${l}.json`), "utf8")),
    ),
  ]),
...
describe("valeurs effectivement traduites", () => {
  it("le catalogue français ne contient pas de mot manifestement anglais", () => {
    const catalog = JSON.parse(
      readFileSync(join(SRC, "locales", "fr.json"), "utf8"),
    );
    ...
    const walk = (obj: Record<string, unknown>, prefix = "") => {
      for (const [k, v] of Object.entries(obj)) {
        ...
        if (v !== null && typeof v === "object")
          ...
          const hit = ENGLISH_ONLY.find((w) =>
            new RegExp(`\\b${w}\\b`, "i").test(v),
          );
          if (hit)
            offenders.push(`${key} = ${JSON.stringify(v)}  (« ${hit} »)`);
        }
      }
    };
    walk(catalog);
...
describe("énumérations traduites", () => {
  it.each(LOCALES)("%s traduit chaque valeur d'énumération", (locale) => {
    ...
    for (const [prefix, values] of Object.entries(DYNAMIC_ENUMS)) {
      for (const value of values) {
        if (!catalogs[locale].has(`${prefix}.${value}`))
  ...
  it("la liste de contrôle couvre les enums réels", () => {
    ...
    const declared = [...bookings.matchAll(/^\s{2}(\w+) = "\w+",$/gm)].map(
      (m) => m[1],
    );
    const resourceTypes = declared.slice(
      0,
      DYNAMIC_ENUMS["bookings.resourceType"].length,
    );
    expect(new Set(resourceTypes)).toEqual(
      new Set(DYNAMIC_ENUMS["bookings.resourceType"]),
    );

    const notices = readFileSync(join(SRC, "lib/api/notices.ts"), "utf8");
    const noticeBlock = notices.slice(
      notices.indexOf("export const NoticeType"),
    );
    const noticeValues = [
      ...noticeBlock.slice(0, noticeBlock.indexOf("}")).matchAll(/(\w+): "/g),
    ].map((m) => m[1]);
    expect(new Set(noticeValues)).toEqual(
      new Set(DYNAMIC_ENUMS["notices.noticeType"]),
    );
  });
});
...
describe("catalogues de traduction", () => {
  it.each(LOCALES)(
    "%s contient toutes les clés utilisées dans le code",
    (locale) => {
      const missing: string[] = [];
      ...
      expect(
        missing,
        `${missing.length} clé(s) absente(s) de ${locale}.json :\n  ${missing.join("\n  ")}`,
      ).toEqual([]);
    },
  );
  ...
  it.each(LOCALES.filter((l) => l !== "fr"))(
    ...
    (locale) => {
      ...
      expect(
        [...fr].filter((k) => !other.has(k)),
        `absentes de ${locale}.json`,
      ).toEqual([]);
      expect(
        [...other].filter((k) => !fr.has(k)),
        `en trop dans ${locale}.json`,
      ).toEqual([]);
    },
  );
});
