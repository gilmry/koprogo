<script lang="ts">
  ...
  async function loadEtatsDates() {
    ...
    const statusFilter = filterStatus
      ? (filterStatus as EtatDateStatus)
      : undefined;
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      <div class="bg-white rounded-lg shadow p-4">
        <p class="text-sm text-gray-600">
          {$_("etatsDate.averageProcessingDays")}
        </p>
        <p class="text-2xl font-bold text-gray-900">
          {stats.average_processing_days.toFixed(1)}j
        </p>
      </div>
    </div>
  {/if}
      ...
      <div>
        ...
        >
          ...
          <option value="in_progress"
            >{$_("etatsDate.status.inProgress")}</option
          >
  ...
  {#if loading}
    <div class="flex justify-center py-12">
      <div
        ...
        data-testid="etat-date-list-spinner"
      ></div>
    </div>
