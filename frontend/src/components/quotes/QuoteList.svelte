<script lang="ts">
  ...
  let statusCounts = $derived({
    ...
    underReview: quotes.filter((q) => q.status === QuoteStatus.UnderReview)
      .length,
...
<div class="bg-white shadow-md rounded-lg" data-testid="quote-list">
        ...
        <div class="flex gap-2">
          ...
          {:else}
            ...
            <button
              ...
              class="px-3 py-1.5 bg-amber-700 text-white rounded-lg text-sm font-medium hover:bg-amber-800 transition-colors"
            >
  ...
  {#if quotes.length > 0}
    <div
      class="px-4 py-2 bg-gray-50 border-b border-gray-200 flex flex-wrap gap-3 text-xs text-gray-600"
    >
      <span>{statusCounts.total} {$_("common.total")}</span>
      {#if statusCounts.requested > 0}<span class="text-blue-600"
          >{statusCounts.requested}
          {$_("quotes.status.requested").toLowerCase()}</span
        >{/if}
      {#if statusCounts.received > 0}<span class="text-purple-600"
          >{statusCounts.received}
          {$_("quotes.status.received").toLowerCase()}</span
        >{/if}
      {#if statusCounts.underReview > 0}<span class="text-yellow-600"
          >{statusCounts.underReview}
          {$_("quotes.status.underReview").toLowerCase()}</span
        >{/if}
      {#if statusCounts.accepted > 0}<span class="text-green-600"
          >{statusCounts.accepted}
          {$_("quotes.status.accepted").toLowerCase()}</span
        >{/if}
      {#if statusCounts.rejected > 0}<span class="text-red-600"
          >{statusCounts.rejected}
          {$_("quotes.status.rejected").toLowerCase()}</span
        >{/if}
    </div>
  {/if}
  ...
  <div class="px-4 py-3 bg-gray-50 border-b border-gray-200">
    <div class="flex items-center space-x-4">
      <label for="quote-status-filter" class="text-sm font-medium text-gray-700"
      ...
      >
        ...
        <option value={QuoteStatus.Requested}
          >{$_("quotes.status.requested")}</option
        >
        <option value={QuoteStatus.Received}
          >{$_("quotes.status.received")}</option
        >
        <option value={QuoteStatus.UnderReview}
          >{$_("quotes.status.underReview")}</option
        >
        <option value={QuoteStatus.Accepted}
          >{$_("quotes.status.accepted")}</option
        >
        <option value={QuoteStatus.Rejected}
          >{$_("quotes.status.rejected")}</option
        >
        <option value={QuoteStatus.Expired}
          >{$_("quotes.status.expired")}</option
        >
        <option value={QuoteStatus.Withdrawn}
          >{$_("quotes.status.withdrawn")}</option
        >
      </select>
    </div>
  </div>
  ...
  {#if showCreateForm}
    <div class="p-4 border-b border-gray-200 bg-amber-50">
      <h4 class="text-sm font-semibold text-gray-900 mb-3">
        {$_("quotes.list.newRequest")}
      </h4>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <label for="contractorId" class="block text-xs text-gray-600 mb-1"
          ...
          <input
            ...
            placeholder={$_("quotes.list.contractorUUID")}
            class="w-full text-sm rounded-md border-gray-300 focus:border-amber-500 focus:ring-amber-500"
          />
        </div>
        <div>
          <label for="workCategory" class="block text-xs text-gray-600 mb-1"
        ...
        <div class="md:col-span-2">
          <label for="projectTitle" class="block text-xs text-gray-600 mb-1"
          ...
          <input
            ...
            placeholder={$_("quotes.list.projectTitlePlaceholder")}
            class="w-full text-sm rounded-md border-gray-300 focus:border-amber-500 focus:ring-amber-500"
          />
        </div>
        <div class="md:col-span-2">
          <label for="projectDesc" class="block text-xs text-gray-600 mb-1"
          ...
          <textarea
            ...
            placeholder={$_("quotes.list.workDetails")}
            class="w-full text-sm rounded-md border-gray-300 focus:border-amber-500 focus:ring-amber-500"
          ></textarea>
        </div>
      </div>
            ...
            >
              <div class="flex items-center space-x-3 flex-1 min-w-0">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div
                    class="flex items-center text-sm text-gray-500 flex-wrap gap-x-4 gap-y-1"
                  >
                    <span
                      >{quote.contractor_name ||
                        quote.contractor_id.slice(0, 8)}</span
                    >
  ...
  {#if quotes.length > 0 && quotes.length < 3}
    <div class="px-4 py-3 bg-yellow-50 border-t border-yellow-200">
      <p class="text-xs text-yellow-800">
        <strong>{$_("quotes.list.bestPracticeTitle")}:</strong>
        {$_("quotes.list.bestPracticeMessage", {
