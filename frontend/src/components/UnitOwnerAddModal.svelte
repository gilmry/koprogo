<script lang="ts">
  ...
  let availablePercentage = $derived(
    Math.max(0, (1 - currentTotalPercentage) * 100),
  );
    ...
    <div class="flex min-h-screen items-center justify-center p-4">
      ...
      >
        <div class="flex justify-between items-center mb-4">
          <h2 id="add-owner-title" class="text-xl font-bold text-gray-900">
            {$_("units.add_owner")}
          </h2>
          <button
            onclick={handleClose}
        ...
        >
          <div>
            <label
              ...
              >{$_("units.owner")} *</label
            >
            {#if loadingOwners}<p class="text-sm text-gray-500">
                {$_("common.loading")}
              </p>
            {:else}
              <label for="owner-search" class="sr-only"
              ...
              <select
                ...
                >{#each filteredOwners as owner (owner.id)}<option
                    value={owner.id}
                    >{owner.first_name}
                    {owner.last_name} ({owner.email})</option
                  >{/each}</select
              >
              {#if filteredOwners.length === 0 && searchQuery}<p
                  class="text-xs text-gray-500 mt-1"
                >
                  {$_("units.no_owner_found", {
                    values: { query: searchQuery },
                  })}
                </p>{:else}<p class="text-xs text-gray-500 mt-1">
                  {$_("units.owners_found", {
                    values: { count: filteredOwners.length },
                  })}
                </p>{/if}
            {/if}
          ...
          <div>
            ...
            {#if wouldExceed}<p class="text-xs text-red-600 mt-1 font-medium">
                {$_("units.quota_would_exceed_detail", {
                  values: {
                    current: (currentTotalPercentage * 100).toFixed(2),
                    added: ownershipPercentage.toFixed(2),
                    total: (
