<script lang="ts">
  ...
  $effect(() => {
    if (buildingIds.length > 0 && !selectedBuildingId)
      selectedBuildingId = buildingIds[0];
  });
...
<div class="space-y-4">
  {#if buildingIds.length === 0}
    <div
      class="bg-gray-50 border border-gray-200 rounded-lg p-4 text-sm text-gray-500"
    >
    ...
    {:else if building}
      ...
      {:else}
        <div class="bg-white rounded-lg border border-gray-200 p-5 space-y-4">
          ...
          <ul class="space-y-4">
            {#if building.syndic_name}<li class="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p class="font-medium text-gray-900">
                    {building.syndic_name}
                  </p>
                </div>
              </li>{/if}
            {#if building.syndic_email}<li class="flex items-start gap-3">
                <span class="text-primary-500 mt-0.5 shrink-0"
                  ><svg
                    class="w-5 h-5"
                    fill="none"
                ...
                <div>
                  ...
                  <a
                    href="mailto:{building.syndic_email}"
                    class="text-primary-600 hover:underline"
                    >{building.syndic_email}</a
                  >
                </div>
              </li>{/if}
            {#if building.syndic_phone}<li class="flex items-start gap-3">
                <span class="text-primary-500 mt-0.5 shrink-0"
                  ><svg
                    class="w-5 h-5"
                    fill="none"
                ...
                <div>
                  ...
                  <a
                    href="tel:{building.syndic_phone}"
                    class="text-primary-600 hover:underline"
                    >{building.syndic_phone}</a
                  >
                </div>
              </li>{/if}
            {#if building.syndic_address}<li class="flex items-start gap-3">
                <span class="text-primary-500 mt-0.5 shrink-0"
                  ><svg
                    class="w-5 h-5"
                    fill="none"
            ...
            {#if building.syndic_office_hours}<li
            ...
            {#if building.syndic_emergency_contact}<li
                class="flex items-start gap-3"
              >
                ...
                <div>
                  <p
                    class="text-xs text-red-400 uppercase tracking-wider font-medium"
                  >
                    {$_("syndic.emergencies")}
                  </p>
                  <p class="text-gray-900 font-medium">
                    {building.syndic_emergency_contact}
                  </p>
                </div>
              </li>{/if}
          </ul>
