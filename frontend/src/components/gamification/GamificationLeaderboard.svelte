...
>
  <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
    ...
    <p class="mt-1 text-sm text-gray-500">
      {$_("gamification.leaderboard_description")}
    </p>
  </div>
  ...
  {:else}
    <div class="divide-y divide-gray-100">
      {#each leaderboard as entry (entry.user_id)}
        ...
        <div
          data-testid="gamification-leaderboard-row"
          class="flex items-center gap-4 px-4 py-3 {isMe
            ? 'bg-amber-50'
            : 'hover:bg-gray-50'}"
        >
          <!-- Rank -->
          <div
            ...
            {entry.rank <= 3
              ? 'bg-gradient-to-br from-yellow-300 to-amber-400 text-white shadow'
            ...
            <div class="flex items-center gap-3 text-xs text-gray-500">
              <span
                >{$_("gamification.achievement_count", {
                  values: { count: entry.achievements_count },
                })}</span
              >
              <span
                >{$_("gamification.challenge_count_completed", {
                  values: { count: entry.challenges_completed },
                })}</span
              >
            </div>
          </div>
