    ...
    switch (status) {
      case ChallengeStatus.Draft:
        return {
          bg: "bg-gray-100",
          text: "text-gray-700",
          label: $_("gamification.status_draft"),
        };
      case ChallengeStatus.Active:
        return {
          bg: "bg-green-100",
          text: "text-green-700",
          label: $_("gamification.status_active"),
        };
      case ChallengeStatus.Completed:
        return {
          bg: "bg-blue-100",
          text: "text-blue-700",
          label: $_("gamification.status_completed"),
        };
      case ChallengeStatus.Cancelled:
        return {
          bg: "bg-red-100",
          text: "text-red-700",
          label: $_("gamification.status_cancelled"),
        };
      default:
...
<div class="bg-white shadow-md rounded-lg" data-testid="admin-challenge-list">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h3 class="text-lg leading-6 font-medium text-gray-900">
          {$_("gamification.challenges_management")}
        </h3>
        <p class="mt-1 text-sm text-gray-500">
          {$_("gamification.challenge_count", {
  ...
  <div class="px-4 py-3 bg-gray-50 border-b border-gray-200">
    <div class="flex flex-wrap gap-1">
      {#each [{ value: "all" as ChallengeStatus | "all", label: $_("common.all") }, { value: ChallengeStatus.Draft as ChallengeStatus | "all", label: $_("gamification.status_drafts") }, { value: ChallengeStatus.Active as ChallengeStatus | "all", label: $_("gamification.status_actives") }, { value: ChallengeStatus.Completed as ChallengeStatus | "all", label: $_("gamification.status_completed_plural") }, { value: ChallengeStatus.Cancelled as ChallengeStatus | "all", label: $_("gamification.status_cancelled_plural") }] as f}
        {@const count =
          f.value === "all"
            ? challenges.length
  ...
  {:else if error}
    <div
      ...
      data-testid="admin-challenge-error"
    >
    ...
    <ul class="divide-y divide-gray-200">
      {#each filteredChallenges as challenge (challenge.id)}
        ...
        <li
          ...
          data-testid="admin-challenge-row"
        >
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1 flex-wrap">
                ...
                <span
                  class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {statusCfg.bg} {statusCfg.text}"
                >
                  {statusCfg.label}
                </span>
                <span class="text-xs text-gray-500"
                  >{typeLabels[challenge.challenge_type]}</span
                >
              </div>
              ...
              <div class="flex items-center gap-3 text-xs text-gray-500">
                ...
                <span
                  >{formatDateShort(challenge.start_date)} - {formatDateShort(
                    challenge.end_date,
                  )}</span
                >
                <span
                  >Objectif: {challenge.target_value}
                  {challenge.target_metric}</span
                >
              </div>
            </div>
