  ...
  async function loadUnits() {
    ...
    try {
      if (selectedBuildingId) {
        const resp = await api.get<Unit[]>(
          `/buildings/${selectedBuildingId}/units`,
        );
        units = Array.isArray(resp) ? resp : [];
      } else {
        const resp = await api.get<{ data: Unit[] }>(
          "/units?page=1&per_page=100",
        );
...
<div class="space-y-4">
  {#if buildingIds.length > 1}
    <div class="flex items-center gap-3">
      ...
      >
        ...
        {#each buildingIds as bid}<option value={bid}
            >{getBuildingName(bid)}</option
          >{/each}
      </select>
    </div>
  {/if}
  ...
  {:else if units.length === 0}
    ...
    >
      ...
      <p class="text-sm">
        {selectedBuildingId
          ? $_("owners.empty.units_building")
          : $_("owners.empty.units_buildings")}
      </p>
    </div>
  {:else}
    <div
      class="bg-white rounded-lg border border-gray-200 p-4 flex flex-wrap gap-6 text-sm"
    >
      <div>
      ...
      <div>
        ...
        ><span class="ml-1 font-semibold text-gray-900"
          >{units.reduce((sum, u) => sum + (u.surface_area || 0), 0)} m²</span
        >
      </div>
      <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {#each units as unit (unit.id)}
        <div
          class="bg-white rounded-lg border border-gray-200 hover:shadow-md transition-shadow p-5"
        >
          <div class="flex items-start justify-between mb-3">
            <div>
              <span
                class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {unit.unit_type ===
                'Apartment'
          ...
          <dl class="space-y-1.5 text-sm">
            ...
            {#if !selectedBuildingId && unit.building_id}<div
              ...
              >
                ...
                <dd
                  class="font-medium text-gray-900 truncate max-w-[160px]"
                  title={getBuildingName(unit.building_id)}
                >
          ...
          {#if unit.owners && unit.owners.length > 0}
            <div class="mt-3 pt-3 border-t border-gray-100">
              ...
              <ul class="space-y-1">
                {#each unit.owners as uo}<li
                    class="text-sm flex justify-between"
                  >
                    <span class="text-gray-700"
                      >{uo.owner?.first_name ?? ""}
                      {uo.owner?.last_name ?? ""}{#if uo.is_primary_contact}
                        <span class="text-xs text-primary-600"
                          >({$_("common.primary_contact")})</span
                        >{/if}</span
                    ><span class="text-gray-500 font-medium"
                      >{(toNumber(uo.ownership_percentage) * 100).toFixed(
                        1,
                      )}%</span
                    >
                  </li>{/each}
