<script lang="ts">
  ...
  async function loadConvocations() {
    await withLoadingState({
      ...
      onSuccess: (data: any) => {
        convocations = extractArray<Convocation>(data, "convocations");
        applyFilters();
      },
...
<div class="bg-white shadow-md rounded-lg" data-testid="convocation-list">
  ...
  <div class="px-4 py-3 bg-gray-50 border-b border-gray-200">
    <div class="flex items-center space-x-4">
      <label
        for="convocation-status-filter"
        class="text-sm font-medium text-gray-700">{$_("common.status")}:</label
      >
      <select
      ...
      >
        ...
        <option value={ConvocationStatus.Draft}
          >{$_("convocations.status.draft")}</option
        >
        <option value={ConvocationStatus.Scheduled}
          >{$_("convocations.status.scheduled")}</option
        >
        <option value={ConvocationStatus.Sent}
          >{$_("convocations.status.sent")}</option
        >
        <option value={ConvocationStatus.Cancelled}
          >{$_("convocations.status.cancelled")}</option
        >
      </select>
    </div>
  </div>
  ...
  {#if loading}
    <div class="p-8 text-center" data-testid="convocation-list-loading">
      <div
        ...
        data-testid="convocation-list-spinner"
      ></div>
    ...
    <ul class="divide-y divide-gray-200" data-testid="convocation-rows">
      {#each filteredConvocations as convocation (convocation.id)}
        ...
        <li
          ...
          data-testid="convocation-row-{convocation.id}"
        >
          <a
            ...
            <div class="flex items-center justify-between">
              <div class="flex-1 min-w-0">
                <div class="flex items-center space-x-3 mb-2">
                  ...
                  <span
                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {statusCfg.bg} {statusCfg.text}"
                    data-testid="convocation-status-{convocation.id}"
                  >
                ...
                <div
                  class="mt-2 flex items-center text-sm text-gray-500 flex-wrap gap-x-4 gap-y-1"
                >
                  <span
                    >📅 {$_("convocations.meetingOn", {
                      values: { date: formatDate(convocation.meeting_date) },
                    })}</span
                  >
                  <span
                    >📧 {convocation.total_recipients}
                    {$_("common.recipient", {
                      values: { count: convocation.total_recipients },
                    })}</span
                  >
                  {#if convocation.opened_count > 0}
                    <span
                      >👁️ {convocation.opened_count}
                      {$_("common.opened", {
                        values: { count: convocation.opened_count },
                      })}</span
                    >
                  {/if}
                  {#if convocation.will_attend_count > 0}
                    <span
                      ...
                      {$_("common.present", {
                        values: { count: convocation.will_attend_count },
                      })}</span
                    >
                  {/if}
