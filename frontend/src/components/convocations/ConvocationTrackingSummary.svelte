<script lang="ts">
  ...
  async function loadSummary() {
    await withLoadingState({
      ...
      onSuccess: (data: TrackingSummary) => {
        summary = data;
      },
...
<div class="bg-white shadow rounded-lg p-6" data-testid="tracking-summary">
  <h3 class="text-lg font-semibold text-gray-900 mb-4">
    {$_("convocations.trackingSummary")}
  </h3>

  {#if loading}
    <div
      class="text-center py-8 text-gray-500"
      data-testid="tracking-summary-loading"
    >
      <div
        class="inline-block animate-spin rounded-full h-6 w-6 border-b-2 border-amber-600"
        data-testid="tracking-summary-spinner"
      ></div>
    ...
    <dl class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <div class="bg-gray-50 rounded-lg p-4" data-testid="tracking-stat-total">
        <dt class="text-sm font-medium text-gray-500 mb-1">
          {$_("convocations.totalRecipients")}
        </dt>
      ...
      {#if summary.email_failed > 0}
        <div
          ...
          data-testid="tracking-stat-failed"
        >
