  ...
  async function loadSession() {
    ...
    try {
      ...
      const res = await fetch(
        apiEndpoint(`/meetings/${meetingId}/ag-session`),
        {
...
<div class="bg-white rounded-lg border border-gray-200 p-4">
  <div class="flex items-center justify-between mb-4">
    ...
    {#if !readOnly && !session && !showForm}
      <Button
        ...
        data-testid="ag-session-create-btn"
      >
  ...
  {:else if showForm}
    <form
      ...
      class="space-y-3"
      data-testid="ag-session-form"
    >
      <div>
        <label
          for="ag-session-platform"
          class="block text-xs font-medium text-gray-700 mb-1"
          >{$_("agSession.platform")}</label
        >
        <select
          id="ag-session-platform"
          bind:value={form.platform}
          class="w-full rounded border border-gray-300 px-2 py-1 text-sm"
          data-testid="ag-session-platform-select"
        >
          {#each platforms as p}
      ...
      <div>
        <label
          for="ag-session-video-url"
          class="block text-xs font-medium text-gray-700 mb-1"
          >{$_("agSession.meetingUrl")} *</label
        >
        <input
          id="ag-session-video-url"
          bind:value={form.video_url}
          type="url"
          required
          class="w-full rounded border border-gray-300 px-2 py-1 text-sm"
          placeholder="https://meet.jit.si/..."
          data-testid="ag-session-video-url-input"
        />
      </div>
      <div>
        <label
          for="ag-session-scheduled-start"
          class="block text-xs font-medium text-gray-700 mb-1"
          >{$_("agSession.scheduledStart")} *</label
        >
        <input
          id="ag-session-scheduled-start"
          bind:value={form.scheduled_start}
          type="datetime-local"
          required
          class="w-full rounded border border-gray-300 px-2 py-1 text-sm"
          data-testid="ag-session-scheduled-start-input"
        />
      </div>
      <div>
        <label
          for="ag-session-host-url"
          class="block text-xs font-medium text-gray-700 mb-1"
          >{$_("agSession.hostUrl")}</label
        >
        <input
          id="ag-session-host-url"
          bind:value={form.host_url}
          type="url"
          class="w-full rounded border border-gray-300 px-2 py-1 text-sm"
          data-testid="ag-session-host-url-input"
        />
      </div>
      <div>
        <label
          for="ag-session-password"
          class="block text-xs font-medium text-gray-700 mb-1"
          >{$_("agSession.password")}</label
        >
        <input
          id="ag-session-password"
          bind:value={form.access_password}
          type="text"
          class="w-full rounded border border-gray-300 px-2 py-1 text-sm"
          data-testid="ag-session-password-input"
        />
      </div>
      <div class="flex gap-4">
        <label class="flex items-center gap-1 text-xs">
          <input
            type="checkbox"
            bind:checked={form.waiting_room_enabled}
            data-testid="ag-session-waiting-room-checkbox"
          />
        ...
        <label class="flex items-center gap-1 text-xs">
          <input
            type="checkbox"
            bind:checked={form.recording_enabled}
            data-testid="ag-session-recording-checkbox"
          />
      ...
      <div class="flex gap-2">
        <Button
          ...
          size="sm"
          loading={creating}
          data-testid="ag-session-submit-btn">{$_("common.create")}</Button
        >
        <Button
          type="button"
          size="sm"
          ...
          data-testid="ag-session-cancel-btn">{$_("common.cancel")}</Button
        >
      </div>
    </form>
  {:else if session}
    <div class="space-y-2">
      <div class="flex items-center gap-2">
        ...
        <span
          class="px-1.5 py-0.5 rounded text-xs font-medium {getStatusBadge(
            session.status,
          )}"
        >
          {session.status}
      ...
      <div class="flex items-center gap-2 text-sm text-gray-600">
        <a
          ...
          data-testid="ag-session-video-link"
        >
          {session.video_url}
        </a>
      </div>
      ...
      {#if session.access_password}
        <p class="text-xs text-gray-500">
          {$_("agSession.password")}:
          <span class="font-mono">{session.access_password}</span>
        </p>
      {/if}
      ...
      <div class="grid grid-cols-3 gap-2 mt-2 text-xs text-gray-600">
        <div class="bg-gray-50 rounded p-1.5 text-center">
          <p class="font-semibold text-gray-900">
            {session.remote_attendees_count}
          </p>
          <p>{$_("agSession.remoteAttendees")}</p>
        </div>
        <div class="bg-gray-50 rounded p-1.5 text-center">
          <p class="font-semibold text-gray-900">
            {(session.remote_voting_power * 100).toFixed(1)}%
          </p>
          <p>{$_("agSession.votingPower")}</p>
        </div>
        <div class="bg-gray-50 rounded p-1.5 text-center">
          <p class="font-semibold text-gray-900">
            {(session.quorum_remote_contribution * 100).toFixed(1)}%
          </p>
        ...
        <div class="flex gap-2 mt-2">
          {#if session.status === "scheduled"}
            <Button
              ...
              data-testid="ag-session-start-btn"
              >▶ {$_("agSession.start")}</Button
            >
          {:else if session.status === "live"}
            <Button
              ...
              variant="danger"
              onclick={endSession}
              data-testid="ag-session-end-btn">⏹ {$_("agSession.end")}</Button
            >
          {/if}
        </div>
      {/if}
