<script lang="ts">
  ...
  let quorumPercentage = $derived(
    totalQuotas > 0 ? (presentQuotas / totalQuotas) * 100 : 0,
  );
  let displayPercentage = $derived(
    (meeting as any).quorum_percentage ?? quorumPercentage,
  );
  ...
  <div class="p-6">
    {#if quorumValidated}
      <div
        ...
        data-testid="quorum-validated-badge"
      >
        ...
        <div>
          ...
          <p class="text-green-700 text-sm">
            {presentQuotas} / {totalQuotas}
            {$_("meetings.quorum.thousandths")}
    ...
    {:else if canManage}
      ...
      <div class="flex items-center justify-between">
        <div class="text-sm text-gray-600">
          {$_("meetings.quorum.preview")}:
          <strong>{quorumPercentage.toFixed(1)}%</strong>
          {#if quorumPercentage > 50}
            <span class="text-green-700"
              >✓ {$_("meetings.quorum.quorum_ok")}</span
            >
          {:else}
            <span class="text-orange-700"
              >⚠ {$_("meetings.quorum.quorum_not_reached")}</span
            >
          {/if}
        </div>
        <button
        ...
        >
          {submitting
            ? $_("meetings.quorum.validating")
            : $_("meetings.quorum.validate")}
        </button>
      </div>
    {:else}
