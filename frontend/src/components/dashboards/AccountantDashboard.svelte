<script lang="ts">
  ...
  function getTransactionLabel(type: string): string {
    return type === "paymentreceived"
      ? $_("dashboards.accountant.transaction.received")
      : $_("dashboards.accountant.transaction.made");
  }
...
<div data-testid="accountant-dashboard">
  <div class="mb-8">
    ...
    <p class="text-gray-600">
      {$_("dashboards.accountant.title")} - {$_(
        "dashboards.accountant.subtitle",
      )}
    </p>
  </div>
  ...
  {:else if stats}
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      <div class="bg-white rounded-lg shadow p-6">
        <div class="flex items-center justify-between mb-2">
          <span class="text-gray-600 text-sm font-medium"
            >{$_("dashboards.accountant.stats.totalExpenses")}</span
          >
          <span class="text-2xl">💰</span>
        </div>
        <p class="text-3xl font-bold text-gray-900">
          {formatCurrency(stats.total_expenses_current_month)}
        </p>
        <p class="text-sm text-gray-500 mt-1">
          {$_("dashboards.accountant.stats.thisMonth")}
        </p>
      </div>
      ...
      <div class="bg-white rounded-lg shadow p-6">
        <div class="flex items-center justify-between mb-2">
          <span class="text-gray-600 text-sm font-medium"
            >{$_("dashboards.accountant.stats.paid")}</span
          >
          <span class="text-2xl">✅</span>
        </div>
        ...
        <p class="text-sm text-gray-500 mt-1">
          {stats.paid_percentage.toFixed(0)}% {$_(
            "dashboards.accountant.stats.collected",
          )}
        </p>
      </div>
      ...
      <div class="bg-white rounded-lg shadow p-6">
        <div class="flex items-center justify-between mb-2">
          <span class="text-gray-600 text-sm font-medium"
            >{$_("dashboards.accountant.stats.pending")}</span
          >
          <span class="text-2xl">⏳</span>
        </div>
        ...
        <p class="text-sm text-gray-500 mt-1">
          {stats.pending_percentage.toFixed(0)}% {$_(
            "dashboards.accountant.stats.remaining",
          )}
        </p>
      </div>
      ...
      <div class="bg-white rounded-lg shadow p-6">
        <div class="flex items-center justify-between mb-2">
          <span class="text-gray-600 text-sm font-medium"
            >{$_("dashboards.accountant.stats.overdue")}</span
          >
          <span class="text-2xl">🚨</span>
        </div>
        <p class="text-3xl font-bold text-red-600">
          {stats.owners_with_overdue}
        </p>
        <p class="text-sm text-gray-500 mt-1">
          {$_("dashboards.accountant.stats.owners")}
        </p>
      </div>
    </div>
  {/if}
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
    ...
    <div class="bg-white rounded-lg shadow">
      <div class="p-6 border-b border-gray-200">
        <h2 class="text-lg font-semibold text-gray-900">
          {$_("dashboards.accountant.recentTransactions")}
        </h2>
      </div>
      <div class="p-6">
        {#if loading}
        ...
        {:else if transactions.length > 0}
          <div class="space-y-4">
            {#each transactions as transaction}
              <div
                class="flex items-center justify-between p-4 border rounded-lg {tonDeLaTransaction(
                  transaction.transaction_type,
                ).cadre}"
              >
                <div class="flex items-center space-x-3">
                  <span class="text-2xl"
                    >{getTransactionIcon(transaction.transaction_type)}</span
                  >
                  <div>
                ...
                <p
                  class="text-lg font-bold {tonDeLaTransaction(
                    transaction.transaction_type,
                  ).montant}"
                >
        ...
        {:else}
          <p class="text-center text-gray-500 py-4">
            {$_("dashboards.accountant.noRecentTransactions")}
          </p>
        {/if}
      </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow">
      <div class="p-6 border-b border-gray-200">
        <h2 class="text-lg font-semibold text-gray-900">
          {$_("dashboards.accountant.quickActions")}
        </h2>
      </div>
      <div class="p-6">
