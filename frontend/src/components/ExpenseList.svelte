<script lang="ts">
  ...
  function getStatusBadge(status: string): { class: string; label: string } {
    const badges: Record<string, { class: string; label: string }> = {
      paid: {
        ...
        label: $_("expenses.statuses.paid"),
      },
      pending: {
        class: "bg-yellow-100 text-yellow-800",
        label: $_("expenses.statuses.pending"),
      },
      overdue: {
        class: "bg-red-100 text-red-800",
        label: $_("expenses.statuses.overdue"),
      },
      cancelled: {
        class: "bg-gray-100 text-gray-800",
        label: $_("expenses.statuses.cancelled"),
      },
    };
    return (
  ...
  function getApprovalBadge(approvalStatus: string): {
    class: string;
    label: string;
    emoji: string;
  } {
    ...
    > = {
      draft: {
        ...
        label: $_("expenses.approval.draft"),
        emoji: "📝",
      },
      pending_approval: {
        class: "bg-blue-100 text-blue-800",
        label: $_("expenses.approval.pendingApproval"),
        emoji: "⏳",
      },
      approved: {
        class: "bg-green-100 text-green-800",
        label: $_("expenses.approval.approved"),
        emoji: "✅",
      },
      rejected: {
        class: "bg-red-100 text-red-800",
        label: $_("expenses.approval.rejected"),
        emoji: "❌",
      },
    };
...
<div class="space-y-4">
  ...
  {#if loading}
    <p class="text-center text-gray-600 py-8" data-testid="loading-spinner">
      {$_("common.loading")}
    </p>
        ...
        >
          <div class="flex justify-between items-start">
            <div class="flex-1">
              <div class="flex items-center gap-2 mb-2 flex-wrap">
                ...
                {#if expense.approval_status}
                  ...
                  <span
                    class="text-xs px-2 py-1 rounded-full {approvalBadge.class}"
                  >
                    {approvalBadge.emoji}
                    {approvalBadge.label}
                  </span>
              ...
              <p class="text-gray-600 text-sm">
                📁 {$_(
                  `invoices.category_${expense.category?.toLowerCase()}`,
                ) || expense.category}
              </p>
            ...
            <div class="text-right">
              ...
              <a
                href="/expense-detail?id={expense.id}"
...
{#if showCreateModal}
  ...
  >
    <div
      class="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto pointer-events-auto"
      role="presentation"
    >
      <div
