<script lang="ts">
  ...
  let {
    reminderId,
    onUpdated = null,
  }: { reminderId: string; onUpdated?: ((reminder: any) => void) | null } =
    $props();
  ...
  async function loadReminder() {
    ...
    const result = await withErrorHandling({
      ...
      errorMessage: $_("reminders.loadFailed"),
    });
    if (result) {
      reminder = result;
    } else {
  ...
  async function confirmCancel() {
    ...
    try {
      ...
      toast.error(err.message || $_("paymentReminders.cancelError"));
    } finally {
      loading = false;
    }
  }
  ...
  async function confirmAddTracking() {
    ...
    try {
      ...
      const updated = await api.put(
        `/payment-reminders/${reminderId}/tracking-number`,
      ...
      toast.error(err.message || $_("paymentReminders.trackingError"));
    } finally {
      loading = false;
    }
  }
  ...
  function getLevelInfo(level: string) {
    const levels: Record<
      string,
      { emoji: string; label: string; description: string; class: string }
    > = {
      FirstReminder: {
        emoji: "📧",
  ...
  function getStatusBadge(status: string) {
    const badges: Record<string, { class: string; label: string }> = {
      Pending: {
        ...
        label: $_("paymentReminders.pendingStatus"),
      },
      Sent: {
        class: "bg-indigo-100 text-indigo-800",
        label: $_("paymentReminders.sentStatus"),
      },
      Opened: {
        class: "bg-purple-100 text-purple-800",
        label: $_("paymentReminders.openedStatus"),
      },
      Paid: {
        class: "bg-green-100 text-green-800",
        label: $_("paymentReminders.paidStatus"),
      },
      Escalated: {
        class: "bg-orange-100 text-orange-800",
        label: $_("paymentReminders.escalatedStatus"),
      },
      Cancelled: {
        class: "bg-gray-100 text-gray-800",
        label: $_("paymentReminders.cancelledStatus"),
      },
    };
    return (
...
{:else if error}<div class="bg-red-50 border-l-4 border-red-400 p-4">
...
{:else if reminder}
  ...
  <div class="space-y-6" data-testid="payment-reminder-detail">
    <div class="bg-white rounded-lg shadow overflow-hidden">
      <div class="border-l-4 {levelInfo.class} border-4 p-6">
        <div class="flex items-start justify-between">
          ...
          <span
            class="inline-flex px-4 py-2 rounded-full text-sm font-medium {statusBadge.class}"
    ...
    <div class="bg-white rounded-lg shadow p-6">
      <h3 class="text-lg font-semibold text-gray-900 mb-4">
        {$_("paymentReminders.amounts")}
      </h3>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p class="text-sm text-gray-600 mb-1">
            {$_("paymentReminders.amountOwed")}
          </p>
        ...
        <div class="bg-red-50 border border-red-200 rounded-lg p-4">
          <p class="text-sm text-gray-600 mb-1">
            {$_("paymentReminders.penaltiesAnnual")}
          </p>
        ...
        <div class="bg-purple-50 border border-purple-200 rounded-lg p-4">
          <p class="text-sm text-gray-600 mb-1">
            {$_("paymentReminders.totalToPay")}
          </p>
          <p class="text-2xl font-bold text-purple-600">
            {formatCurrency(reminder.total_amount)}
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
      <h3 class="text-lg font-semibold text-gray-900 mb-4">
        {$_("paymentReminders.deliveryMethod")}
      </h3>
      <div class="space-y-3">
        <div class="flex items-center justify-between py-2">
          <span class="text-gray-600">{$_("paymentReminders.method")}</span
          ><span class="font-medium text-gray-900"
            >{#if reminder.delivery_method === "Email"}📧 {$_(
                "paymentReminders.methodEmail",
              )}{:else if reminder.delivery_method === "RegisteredLetter"}📮 {$_(
                "paymentReminders.methodRegistered",
              )}{:else if reminder.delivery_method === "Bailiff"}⚖️ {$_(
                "paymentReminders.methodBailiff",
              )}{:else}{reminder.delivery_method}{/if}</span
          >
        </div>
        {#if reminder.tracking_number}<div
            class="flex items-center justify-between py-2 bg-yellow-50 px-3 rounded"
          >
            <span class="text-gray-600"
              >{$_("paymentReminders.trackingNumber")}</span
    ...
    {#if reminder.notes}<div class="bg-white rounded-lg shadow p-6">
  ...
  {#if showCancelModal}<div
      class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
    >
      <div class="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">
          {$_("paymentReminders.cancelTitle")}
        </h3>
        <div class="mb-4">
          <label
            for="cancel-reason"
