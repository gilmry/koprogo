<script lang="ts">
  ...
  let pendingCount = $derived(
    resolutions.filter((r) => r.status === ResolutionStatus.Pending).length,
  );
  let adoptedCount = $derived(
    resolutions.filter((r) => r.status === ResolutionStatus.Adopted).length,
  );
  let rejectedCount = $derived(
    resolutions.filter((r) => r.status === ResolutionStatus.Rejected).length,
  );
</script>
...
>
    ...
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-xl font-semibold text-white">
          {$_("resolutions.list.title")}
        </h2>
        {#if resolutions.length > 0}
          <p class="text-indigo-200 text-sm mt-1">
            {resolutions.length}
            {$_("resolutions.list.resolution", {
              values: { count: resolutions.length },
            })}
            {#if pendingCount > 0}
              &middot; <span data-testid="resolution-pending-count"
                >{pendingCount} {$_("resolutions.list.pending")}</span
              >
            {/if}
            {#if adoptedCount > 0}
              &middot; <span data-testid="resolution-adopted-count"
                >{adoptedCount}
                {$_("resolutions.list.adopted", {
                  values: { count: adoptedCount },
                })}</span
              >
            {/if}
            {#if rejectedCount > 0}
              &middot; <span data-testid="resolution-rejected-count"
                >{rejectedCount}
                {$_("resolutions.list.rejected", {
                  values: { count: rejectedCount },
                })}</span
              >
            {/if}
          </p>
  ...
  <div class="p-6">
          ...
          <div class="relative" data-testid="resolution-item">
            <div class="flex items-start gap-2 mb-2">
              ...
              {#if resolution.is_auto_generated}
                <span
                  class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-indigo-100 text-indigo-800 shrink-0"
                  data-testid="resolution-auto-badge"
                  title={$_("resolutions.list.autoGeneratedBadge")}
                >
                  {$_("resolutions.list.autoGeneratedBadge")}
                </span>
              {/if}
              {#if isAdmin && resolution.status === ResolutionStatus.Pending && !resolution.is_auto_generated}
                <button
    ...
    {#if resolutions.length > 0 || showCreateForm}
      <div
        class="mt-6 p-3 bg-blue-50 border-l-4 border-blue-400 rounded-md text-xs text-blue-800"
      >
        <strong>{$_("resolutions.list.belgianLaw")}:</strong>
        {$_("resolutions.list.legalText")}
      </div>
    {/if}
  </div>
</div>
