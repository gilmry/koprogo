<script lang="ts">
  ...
  export let status: ResolutionStatus;

  const config: Record<ResolutionStatus, { bg: string; text: string; label: string; icon: string }> = {
  ...
  $: badge = config[status] || config[ResolutionStatus.Pending];
</script>
