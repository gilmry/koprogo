<script lang="ts">
  ...
  let {
    ownerId,
    showHistory = false,
  }: { ownerId: string; showHistory?: boolean } = $props();
...
<div class="space-y-4" data-testid="owner-units">
  ...
  {:else}
              ...
              <div class="flex-1">
                {#if ownerUnit.unit}
                  <div class="flex items-center gap-2">
                    <span class="text-2xl"
                      >{getUnitTypeIcon(ownerUnit.unit.unit_type)}</span
                    >
                    <div>
                      <h5 class="font-semibold text-gray-900">
                        {$_("common.unit")}
                        {ownerUnit.unit.unit_number}
                      </h5>
                      <p class="text-sm text-gray-600">
                        {getUnitTypeLabel(ownerUnit.unit.unit_type)} - {$_(
                          "common.floor",
                        )}
                  ...
                  {#if ownerUnit.is_primary_contact}<span
                      class="inline-block mt-2 px-2 py-0.5 text-xs font-medium bg-primary-100 text-primary-800 rounded-full"
                ...
                <p class="text-xs text-gray-500 mt-1">
                  {$_("owners.since")}
                  {formatDate(ownerUnit.start_date)}
                </p>
              </div>
              <div class="ml-4 text-right">
                <p class="text-2xl font-bold text-primary-600">
                  {formatPercentage(toNumber(ownerUnit.ownership_percentage))}
                </p>
    ...
    {:else}<p class="text-center text-gray-600 py-4">
    ...
    {#if showHistory && inactiveUnits.length > 0}
      <div class="space-y-2 mt-6">
        <h4 class="text-sm font-semibold text-gray-700 uppercase tracking-wide">
          {$_("owners.history_units", {
            values: { count: inactiveUnits.length },
          })}
        </h4>
          ...
          >
            <div class="flex justify-between items-start">
              <div class="flex-1">
                ...
                <p class="text-xs text-gray-500 mt-1">
                  {formatDate(ownerUnit.start_date)} → {ownerUnit.end_date
                    ? formatDate(ownerUnit.end_date)
                    : $_("common.ongoing")}
                </p>
              </div>
              <div class="ml-4 text-right">
