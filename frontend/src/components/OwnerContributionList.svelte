<script lang="ts">
  ...
  let {
    ownerId = null,
    organizationId,
  }: { ownerId?: string | null; organizationId: string } = $props();
  ...
  async function recordPayment() {
    ...
    await withErrorHandling({
      action: async () => {
        ...
        await api.put(
          `/owner-contributions/${selectedContribution.id}/mark-paid`,
          payload,
        );
      },
...
<div class="bg-white shadow-md rounded-lg">
  ...
  {#if loading}<div class="p-8 text-center text-gray-500">
      {$_("common.loading")}
    </div>
  {:else if error}<div class="p-4 bg-red-50 border border-red-200 text-red-700">
      {error}
    </div>
  {:else if contributions.length === 0}<div
        ...
        <thead class="bg-gray-50"
          ><tr>
            <th
              ...
              >{$_("contributions.date")}</th
            >
            <th
              scope="col"
              class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
              >{$_("common.description")}</th
            >
            <th
              scope="col"
              class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
              >{$_("contributions.type")}</th
            >
            <th
              scope="col"
              class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
              >{$_("contributions.amount")}</th
            >
            <th
              scope="col"
              class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
              >{$_("contributions.status")}</th
            >
            <th
              scope="col"
              class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
          ...
          {#each contributions as contribution}
            <tr class="hover:bg-gray-50" data-testid="contribution-row">
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"
                >{formatDateShort(contribution.contribution_date)}</td
              >
              <td class="px-6 py-4 text-sm text-gray-900"
                >{contribution.description}</td
              >
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600"
                >{getTypeLabel(contribution.contribution_type)}</td
              >
              <td
                class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"
                >{formatCurrency(contribution.amount)}</td
              >
              <td class="px-6 py-4 whitespace-nowrap"
                ><span
                  ...
                  data-testid="contribution-status-badge"
                  >{getStatusLabel(contribution.payment_status)}</span
                ></td
              >
              <td class="px-6 py-4 whitespace-nowrap text-sm">
                {#if contribution.payment_status === "pending"}
                  <button
                    onclick={() => openPaymentModal(contribution)}
                    class="text-blue-600 hover:text-blue-900 font-medium"
                    data-testid="record-payment-button"
                    >{$_("contributions.recordPayment")}</button
                  >
                {:else if contribution.payment_date}
                  <span class="text-green-600 text-xs"
                    >{$_("contributions.paidOn")}
                    {formatDateShort(contribution.payment_date)}</span
                  >
                {/if}
              </td>
            </tr>
          ...
          <div>
            ...
            ><select
              ...
              ><option value="bank_transfer"
                >{$_("contributions.methodBank")}</option
              ...
              ><option value="domiciliation"
                >{$_("contributions.methodDomicile")}</option
              ></select
            >
          </div>
          <div>
