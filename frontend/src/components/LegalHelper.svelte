<script lang="ts">
  ...
  function getContextualContent() {
    ...
    if (currentPath.includes("/meetings")) {
      return {
        title: $_("legal.meetings.title", {
          default: "General Assembly Rules",
        }),
        content: "ag_sequence",
      };
    } else if (
      currentPath.includes("/resolutions") ||
      currentPath.includes("/votes")
    ) {
      return {
        title: $_("legal.resolutions.title", {
          default: "Voting & Majorities",
        }),
        content: "majorities",
      };
    } else if (
      currentPath.includes("/expenses") ||
      currentPath.includes("/travaux")
    ) {
      return {
      ...
      return {
        title: $_("legal.default.title", {
          default: "Belgian Copropriété Law",
        }),
        content: "general",
      };
    }
...
{#if isOpen}
  ...
  <!-- Panel -->
  <div
    class="fixed right-0 top-0 h-screen w-96 bg-white shadow-2xl z-40 overflow-y-auto flex flex-col"
  >
    <!-- Header -->
    <div
      class="sticky top-0 bg-gradient-to-r from-blue-600 to-blue-700 text-white p-4 shadow-md flex items-center justify-between"
    >
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-4">
      ...
      {:else if contextContent.content === "ag_sequence"}
        ...
        <div class="space-y-3">
          <p class="text-sm text-gray-600 mb-4">
            {$_("legal.ag_sequence.description", {
              default:
                "Mandatory sequence of general assembly agenda items according to Belgian law",
            })}
          </p>
          {#each agSequence as step (step.step)}
            <div
              class="border-l-4 border-blue-500 pl-3 py-2 bg-blue-50 rounded"
            >
              <div class="flex items-start justify-between gap-2">
                <div class="flex-1">
                  ...
                  {#if step.mandatory}
                    <span
                      class="inline-block mt-1 px-2 py-0.5 bg-red-100 text-red-700 text-xs rounded"
                    >
                  ...
                  {#if step.majority}
                    <span
                      class="inline-block mt-1 ml-2 px-2 py-0.5 bg-amber-100 text-amber-700 text-xs rounded"
                    >
                      {step.majority}
                    </span>
                  {/if}
      ...
      {:else if contextContent.content === "majorities"}
        ...
        <div class="space-y-3">
          <p class="text-sm text-gray-600 mb-4">
            {$_("legal.majorities.description", {
              default: "Majority rules for different types of decisions",
            })}
          </p>
          {#each majorityTypes as majority (majority.decision_type)}
            <div class="border rounded-lg p-3 bg-gray-50">
              ...
              <p class="text-xs text-gray-600 mt-1">
                {majority.threshold_description}
              </p>
                ...
                <div class="mt-2 bg-white p-2 rounded text-xs">
                  ...
                  <div class="w-full bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      style="width: {majority.percentage}%"
                    ></div>
                  </div>
                </div>
              {/if}
              {#if majority.examples && majority.examples.length > 0}
                <p class="text-xs text-gray-700 mt-2">
                  <span class="font-semibold"
                    >{$_("legal.examples", { default: "Examples" })}:</span
                  >
      ...
      {:else if contextContent.content === "travaux"}
        ...
        <div class="space-y-3">
          <p class="text-sm text-gray-600 mb-4">
            {$_("legal.travaux.description", {
              default: "Rules for works and expenses",
            })}
          </p>
          {#each getRelevantRules("travaux") as rule (rule.code)}
            <div
              class="border-l-4 border-orange-500 pl-3 py-2 bg-orange-50 rounded"
            >
              <p class="font-semibold text-sm text-gray-900">
      ...
      {:else}
        ...
        <div class="space-y-3">
          <p class="text-sm text-gray-600 mb-4">
            {$_("legal.general.description", {
              default: "General information about Belgian copropriété law",
            })}
          </p>
          {#each getRelevantRules().slice(0, 5) as rule (rule.code)}
            <div
              class="border-l-4 border-green-500 pl-3 py-2 bg-green-50 rounded"
            >
              <p class="font-semibold text-sm text-gray-900">
                {rule.code} — {rule.title}
              </p>
    ...
    <div class="sticky bottom-0 bg-gray-100 border-t p-3 text-xs text-gray-600">
      <p>
        {$_("legal.footer", {
          default:
            "Based on Belgian Civil Code Articles 3.78-3.100 and IPI Ethics Code",
        })}
      </p>
    </div>
  </div>
