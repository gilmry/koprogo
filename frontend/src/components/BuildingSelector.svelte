...
{#if loading}
  <div class="text-sm text-gray-500 py-2" data-testid="loading-spinner">
    {$_("buildings.loading")}
  </div>
{:else if error}
  <div
    class="p-3 bg-red-50 border border-red-200 rounded-md"
    data-testid="building-selector-error"
  >
...
{:else if buildings.length === 1}
  <div data-testid="building-selected">
    ...
    <div
      class="mt-1 px-3 py-2 bg-gray-50 border border-gray-200 rounded-md text-sm text-gray-700"
    >
      {buildings[0].name} — {buildings[0].address}{#if buildings[0].city}, {buildings[0]
          .postal_code}
        {buildings[0].city}{/if}
    </div>
  </div>
        ...
        <optgroup label={groupe.nom}>
          {#each groupe.blocs as building (building.id)}
            <option value={building.id}>
              {building.name} — {building.address}{#if building.city}, {building.postal_code}
                {building.city}{/if}
            </option>
          {/each}
        </optgroup>
