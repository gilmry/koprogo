<script lang="ts">
  ...
  let {
    ...
    open?: boolean;
    unitOwner?: (UnitOwner & { owner?: Owner }) | null;
    currentTotalPercentage?: number;
    onupdated?: () => void;
    onclose?: () => void;
  } = $props();

  let ownershipPercentage = $state(0);
  let isPrimaryContact = $state(false);
  let loading = $state(false);
  let error = $state("");

  // `ownership_percentage` arrive en STRING JSON (Decimal cote Rust) : on
  // convertit ICI, sinon la soustraction de la ligne suivante donne NaN et le
  // garde-fou de depassement du formulaire ne garde plus rien.
  let currentOwnerPercentage = $derived(
    unitOwner ? toNumber(unitOwner.ownership_percentage) : 0,
  );
  let totalWithoutCurrent = $derived(
    currentTotalPercentage - currentOwnerPercentage,
  );
  let availablePercentage = $derived(
    Math.max(0, (1 - totalWithoutCurrent) * 100),
  );
  let wouldExceed = $derived(
    ownershipPercentage > 0 && ownershipPercentage > availablePercentage + 0.01,
  );

  $effect(() => {
    if (unitOwner && open) {
      ownershipPercentage = toNumber(unitOwner.ownership_percentage) * 100;
  ...
  async function handleSubmit() {
    ...
    if (ownershipPercentage > availablePercentage + 0.01) {
      error = $_("units.quota_would_exceed", {
        values: { available: availablePercentage.toFixed(2) },
      });
      return;
    }
    if (!unitOwner) {
  ...
  <div class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex min-h-screen items-center justify-center p-4">
      ...
      >
        ...
        {#if unitOwner?.owner}<div class="mb-4 p-3 bg-gray-50 rounded-lg">
        ...
        >
          <div>
            ...
            <div class="flex justify-between items-center mt-1">
              <p class="text-xs text-gray-500">{$_("units.quota_sum_100")}</p>
              <p
                class="text-xs font-semibold"
                class:text-green-600={availablePercentage > 0}
                class:text-red-600={availablePercentage <= 0}
              >
                {$_("units.maximum", {
                  values: { pct: availablePercentage.toFixed(2) },
                })}
              </p>
            </div>
            {#if wouldExceed}<p class="text-xs text-red-600 mt-1 font-medium">
          ...
          <div class="flex gap-2 pt-4">
            <Button
              ...
              >{loading ? $_("common.saving") : $_("common.save")}</Button
