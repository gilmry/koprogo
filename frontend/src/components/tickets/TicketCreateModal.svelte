<script lang="ts">
  ...
  $effect(() => {
    if (requesterId && !formData.requester_id)
      formData.requester_id = requesterId;
  });
  $effect(() => {
...
<Modal isOpen={open} onclose={handleClose} title={$_("tickets.createTitle")}>
  ...
  >
    <div class="space-y-4 p-6 pb-4">
      ...
      {#if !buildingId}
        <div>
          ...
          {#if loadingBuildings}
            <p class="text-sm text-gray-500" data-testid="loading-spinner">
              {$_("buildings.loading")}
            </p>
          {:else}
            <select
            ...
            >
              ...
              {#each buildings as building (building.id)}
                <option value={building.id}>
                  {building.name} - {building.address}, {building.postal_code}
                  {building.city}
                </option>
              {/each}
            </select>
      ...
      <FormSelect
        ...
        options={[
          {
            value: TicketCategory.Plumbing,
            label: $_("tickets.categories.plumbing"),
          },
          {
            value: TicketCategory.Electrical,
            label: $_("tickets.categories.electrical"),
          },
          {
            value: TicketCategory.Heating,
            label: $_("tickets.categories.heating"),
          },
          {
            value: TicketCategory.CommonAreas,
            label: $_("tickets.categories.commonAreas"),
          },
          {
            value: TicketCategory.Elevator,
            label: $_("tickets.categories.elevator"),
          },
          {
            value: TicketCategory.Security,
            label: $_("tickets.categories.security"),
          },
          {
            value: TicketCategory.Cleaning,
            label: $_("tickets.categories.cleaning"),
          },
          {
            value: TicketCategory.Landscaping,
            label: $_("tickets.categories.landscaping"),
          },
          {
            value: TicketCategory.Other,
            label: $_("tickets.categories.other"),
          },
        ]}
      />
    ...
    <div
      class="sticky bottom-0 bg-gray-50 border-t border-gray-200 p-4 flex justify-end space-x-3"
    >
      <Button
        type="button"
        variant="outline"
