<script lang="ts">
  ...
  import { skillsApi, type SkillOffer } from "../../lib/api/skills";
  ...
  let offer = $state<SkillOffer | null>(null);
  ...
  let isOwner = $derived(offer && offer.owner_id === currentUserId);
</script>

<div
  class="bg-white shadow rounded-lg overflow-hidden"
  data-testid="skill-offer-detail"
>
  {#if loading}
    ...
    <div class="p-6">
        ...
        {#if offer.is_free}
          <div class="bg-blue-50 px-4 py-2 rounded-lg">
            <p class="text-sm font-semibold text-blue-700">
              {$_("skills.detail.free")}
            </p>
          </div>
        {:else if offer.hourly_rate_credits}
          <div class="bg-green-50 px-4 py-2 rounded-lg">
            <p class="text-lg font-bold text-green-700">
              {offer.hourly_rate_credits}
              {$_("skills.detail.creditsPerHour")}
            </p>
          </div>
        {/if}
      </div>
      ...
      <div class="grid grid-cols-2 gap-4 mb-6 bg-gray-50 rounded-lg p-4">
        <div class="text-center">
          <p class="text-sm font-medium text-gray-900">
            {offer.is_available_for_help
              ? $_("skills.detail.available")
              : $_("skills.detail.unavailable")}
          </p>
          <p class="text-sm text-gray-600">{$_("common.status")}</p>
        </div>
        <div class="text-center">
          <p class="text-sm font-medium text-gray-900">
            {offer.is_professional
              ? $_("skills.detail.professional")
              : $_("skills.detail.community")}
          </p>
      ...
      <div class="grid md:grid-cols-2 gap-6 mb-6">
        {#if offer.years_of_experience}
          <div>
            <h3 class="text-sm font-semibold text-gray-900 mb-2">
              {$_("skills.detail.experience")}
            </h3>
            <p class="text-gray-700">
              {offer.years_of_experience}
              {$_("skills.detail.years")}
            </p>
          </div>
        {/if}
        ...
        {#if offer.certifications}
          <div>
            <h3 class="text-sm font-semibold text-gray-900 mb-2">
              {$_("skills.detail.certifications")}
            </h3>
