      ...
      {#if isOwner}
        <div class="flex gap-2">
          {#if offer.is_available_for_help}
            <button
              onclick={async () => { if (offer) { await skillsApi.markUnavailable(offer.id); await loadOfferDetails(); } }}
          ...
          {:else}
            <button
              onclick={async () => { if (offer) { await skillsApi.markAvailable(offer.id); await loadOfferDetails(); } }}
