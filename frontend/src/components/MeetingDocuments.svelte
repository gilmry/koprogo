<script lang="ts">
  ...
  let { meetingId, meetingStatus }: { meetingId: string; meetingStatus: string } = $props();
  ...
  let uploadDocumentType = $state('MeetingMinutes');
  ...
  function handleFileChange(event: Event) {
    ...
    if (target.files && target.files.length > 0) uploadFile = target.files[0];
  }

  async function handleUpload() {
    if (!uploadFile || !uploadTitle) { toast.error($_('meetings.fill_required_fields')); return; }
    ...
    await withErrorHandling({
      action: async () => {
        ...
        const uploadedDoc = await api.uploadDocument({ file, buildingId, documentType: uploadDocumentType as any, title: uploadTitle, description: uploadDescription || undefined, uploadedBy: userInfo.id });
        await api.put(`/documents/${uploadedDoc.id}/link-meeting`, { meeting_id: meetingId });
        return uploadedDoc;
      },
      setLoading: (v: boolean) => uploading = v,
      successMessage: $_('meetings.document_added'),
      errorMessage: $_('meetings.error_uploading'),
      onSuccess: () => { uploadFile = null; uploadTitle = ''; uploadDescription = ''; uploadDocumentType = 'MeetingMinutes'; showUploadForm = false; loadDocuments(); },
    });
  }
  ...
  function getDocumentTypeLabel(type: string): string {
    const labels: Record<string, string> = { 'MeetingMinutes': $_('meetings.doc_type_minutes'), 'FinancialStatement': $_('meetings.doc_type_financial'), 'Invoice': $_('meetings.doc_type_invoice'), 'Contract': $_('meetings.doc_type_contract'), 'Regulation': $_('meetings.doc_type_regulation'), 'WorksQuote': $_('meetings.doc_type_quote'), 'Other': $_('meetings.doc_type_other') };
...
<div class="bg-white rounded-lg shadow p-6">
    ...
    <div class="bg-gray-50 rounded-lg p-4 mb-6">
      ...
      <div class="space-y-4">
        ...
        <div class="flex gap-2">
          <Button variant="primary" onclick={handleUpload} disabled={uploading}>{uploading ? $_('common.uploading') : $_('meetings.add_document')}</Button>
          <Button variant="outline" onclick={() => showUploadForm = false}>{$_('common.cancel')}</Button>
        </div>
      </div>
    </div>
  {/if}
  ...
  {#if loading}
    <div class="text-center text-gray-500 py-8" data-testid="meeting-documents-loading"><p>{$_('meetings.loading_documents')}</p></div>
  {:else if documents.length === 0}
    <div class="text-center text-gray-500 py-8"><p>{$_('meetings.no_documents')}</p><p class="text-sm mt-2">{$_('meetings.add_documents_help')}</p></div>
  {:else}
          ...
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <div class="flex items-center gap-2 mb-1">
                ...
                <span class="text-xs px-2 py-1 rounded-full bg-blue-100 text-blue-800">{getDocumentTypeLabel(doc.document_type)}</span>
              </div>
              {#if doc.description}<p class="text-sm text-gray-600 ml-8">{doc.description}</p>{/if}
              <div class="flex gap-4 text-xs text-gray-500 ml-8 mt-1">
                <span>📅 {formatDate(doc.created_at)}</span>
                <span>💾 {formatFileSize(doc.file_size)}</span>
              </div>
            </div>
            <Button variant="outline" onclick={() => handleDownload(doc.id, doc.title)} data-testid="document-download-btn">{$_('common.download')}</Button>
          </div>
        </div>
      {/each}
    </div>
