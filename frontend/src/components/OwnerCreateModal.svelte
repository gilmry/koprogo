...
{#if isOpen}
  ...
  >
    <div
      class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto pointer-events-auto"
    >
      ...
      <form
        ...
        class="p-6 space-y-4"
      >
        {#if error}<div class="bg-red-50 border border-red-200 rounded-lg p-3">
        ...
        {#if isSuperAdmin}
          <div>
            ...
            <select
              ...
              ><option value=""
                >{$_("common.action.select_organization")}</option
              >{#each organizations as org}<option value={org.id}
                  >{org.name}</option
                >{/each}</select
            >
          </div>
        ...
        <div class="flex justify-end space-x-3 pt-4">
          ...
          <button
            ...
            >{loading
              ? $_("common.action.creating")
              : $_("owners.create.action")}</button
          >
        </div>
      </form>
    </div>
