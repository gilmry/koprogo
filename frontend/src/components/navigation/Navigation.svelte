<script lang="ts">
  ...
  //
  // Refacto majeur du composant historique `components/Navigation.svelte` :
  ...
  // - chrome desktop sidebar + mobile drawer (UX identique a l'historique)
  // - NotificationBell + role selector multi-organisation + logout
  // - skip-link et a11y a11y baseline WCAG 2.1 AA
  //
  // Pourquoi etendre l'existant et pas tout casser :
  // - Layout.astro + tests caracterisation E2E dependent du chrome actuel
  // - Refonte cosmetique = story 2.5+ (deep-links + tabs contextuels)
  ...
  // ---------------------------------------------------------------------------
  // Mapping rôle → libellés + URLs.
  // Source de verite UNIQUE des items par menu. canSee() decide juste si on
  // affiche le menu ; ici on dit QUOI mettre dedans.
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Resolution scope reactif pour canSee
  // ---------------------------------------------------------------------------

  let currentScope = $derived({
    selectedBuildingId: scope.selectedBuildingId,
    selectedAcpId: scope.selectedAcpId,
    selectedPortfolioId: scope.selectedPortfolioId,
  });

  // Helper raccourci dans le markup
  const see = (menu: Menu) => canSee(activeRole as any, menu, currentScope);
    ...
    if (!success) {
      ...
      if (nextUser?.role) {
        ...
        if (
          ...
          !window.location.pathname.startsWith(destination)
        ) {
...
{#if isAuthenticated}
  ...
  {:else}
    ...
    >
      ...
      <!-- Menus role-conditionnes via canSee -->
      <nav
      ...
      >
        ...
        {#if see("gestion")}
          <RoleSubmenu
            menuKey="gestion"
            title={$_("navigation.management")}
            items={getGestionItems($_)}
            {currentPath}
          />
        {/if}
        ...
        {#if see("compta")}
          <RoleSubmenu
            menuKey="compta"
            title={$_("navigation.accounting")}
            items={getComptaItems($_)}
            {currentPath}
          />
        {/if}
        ...
        {#if see("gouvernance")}
          <RoleSubmenu
            menuKey="gouvernance"
            title={$_("navigation.governance")}
            items={getGouvernanceItems($_)}
            {currentPath}
          />
        {/if}
        ...
        {#if see("ticketing")}
          <RoleSubmenu
            menuKey="ticketing"
            title={$_("navigation.tickets")}
            items={getTicketingItems($_)}
            {currentPath}
          />
        {/if}
        ...
        {#if see("communaute")}
          <RoleSubmenu
            menuKey="communaute"
            title={$_("navigation.community")}
            items={getCommunauteItems($_)}
            {currentPath}
          />
        {/if}
        ...
        {#if see("mes-lots")}
          <RoleSubmenu
            menuKey="mes-lots"
            title={$_("navigation.mySpace")}
            items={getMesLotsItems($_)}
            {currentPath}
          />
        {/if}
      </nav>
      ...
      >
        ...
        <div class="flex items-center gap-2.5 mb-2">
          <div
            class="w-8 h-8 rounded-full bg-primary-600 text-white flex items-center justify-center font-semibold text-xs shrink-0"
            aria-hidden="true"
          >
  ...
  >
    ...
    <div class="shrink-0 border-t border-gray-200 p-4">
      ...
      <div class="flex items-center gap-2.5 mb-3">
        <div
          class="w-9 h-9 rounded-full bg-primary-600 text-white flex items-center justify-center font-semibold text-sm shrink-0"
          aria-hidden="true"
        >
          {user?.first_name?.[0]}{user?.last_name?.[0]}
        </div>
