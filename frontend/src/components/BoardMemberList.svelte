<script lang="ts">
  ...
  function getMandateStatusText(member: BoardMemberResponse): string {
    ...
    if (member.expires_soon)
      return $_("board.status.expiresSoon", {
        values: { days: member.days_remaining },
      });
    ...
    <p class="mt-1 text-sm text-gray-600">
      {$_("board.memberCount", {
        values: {
          ...
          status: showInactive ? "total" : "active",
        },
      })}
    </p>
  </div>
      ...
      {#each members as member}
        <li class="px-6 py-4 hover:bg-gray-50">
          <div class="flex items-center justify-between">
            <div class="flex items-center flex-1">
              <span class="text-3xl mr-4"
                >{getPositionIcon(member.position)}</span
              >
              <div class="flex-1">
                <div class="flex items-center">
                  ...
                  <span
                    class="ml-3 inline-flex items-center px-3 py-0.5 rounded-full text-sm font-medium bg-primary-100 text-primary-800"
                  >
                ...
                <div class="mt-2 text-sm text-gray-600 space-y-1">
                  <p>
                    ...
                    {formatDate(member.mandate_start)} → {formatDate(
                      member.mandate_end,
                    )}
                  </p>
                  <p>
                    <strong>{$_("board.electedAt")}:</strong> AG du {formatDate(
                      (member as any).elected_at ?? member.mandate_start,
                    )}
                  </p>
                </div>
              </div>
            ...
            <div class="ml-4 flex-shrink-0">
              <span
                class="inline-flex items-center px-3 py-1.5 rounded-md text-sm font-medium border {getMandateStatusColor(
                  member,
                )}"
              >
          ...
          {#if member.expires_soon && member.is_active}
            <div
              class="mt-3 bg-orange-50 border border-orange-200 rounded-md p-3"
            >
              <div class="flex">
  ...
  <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
    <div class="flex items-center justify-between">
      <p class="text-sm text-gray-600">
        <strong>{$_("board.legalNote")}:</strong>
