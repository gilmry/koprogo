    ...
    await withLoadingState({
      action: async () => {
        ...
        if (b?.status === "Approved") {
          try {
            v = await budgetsApi.getVariance(budgetId);
          } catch {
            /* not available */
          }
        }
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-primary-100 mt-1">
              {$_("budgets.building")}: {buildingName ||
    ...
    >
      <div class="bg-white rounded-lg shadow p-6">
        ...
        <p class="text-2xl font-bold text-gray-900">
          {formatCurrency(budget.ordinary_budget)}
        </p>
        <p class="text-xs text-gray-500 mt-1">
          {$_("budgets.ordinaryBudgetHint")}
        </p>
      </div>
      <div class="bg-white rounded-lg shadow p-6">
        ...
        <p class="text-2xl font-bold text-gray-900">
          {formatCurrency(budget.extraordinary_budget)}
        </p>
        <p class="text-xs text-gray-500 mt-1">
          {$_("budgets.extraordinaryBudgetHint")}
        </p>
      </div>
      <div class="bg-white rounded-lg shadow p-6">
      ...
      <div class="bg-white rounded-lg shadow p-6">
        <p class="text-sm text-gray-600 mb-1">
          {$_("budgets.monthlyProvision")}
        </p>
        <p class="text-2xl font-bold text-green-600">
          {formatCurrency(budget.monthly_provision_amount)}
        </p>
        <p class="text-xs text-gray-500 mt-1">
          {formatCurrency(budget.total_budget)} / 12
        </p>
      </div>
    </div>
        ...
        {#if budget.approved_date}
          <div class="flex justify-between py-2 border-b">
            ...
            <span class="font-medium text-green-600"
              >{formatDate(budget.approved_date)}</span
            >
          </div>
        {/if}
        {#if budget.notes}
      ...
      <div class="bg-white rounded-lg shadow p-6" data-testid="budget-variance">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div>
            ...
            <p
              class="text-lg font-bold {variance.variance_ordinary >= 0
                ? 'text-green-600'
                : 'text-red-600'}"
            >
              {variance.variance_ordinary >= 0 ? "+" : ""}{formatCurrency(
                variance.variance_ordinary,
              )}
              <span class="text-sm font-normal"
                >({variance.variance_ordinary_pct.toFixed(1)}%)</span
              >
            </p>
          </div>
          <div>
            ...
            <p
              class="text-lg font-bold {variance.variance_extraordinary >= 0
                ? 'text-green-600'
                : 'text-red-600'}"
            >
              {variance.variance_extraordinary >= 0 ? "+" : ""}{formatCurrency(
                variance.variance_extraordinary,
              )}
              <span class="text-sm font-normal"
                >({variance.variance_extraordinary_pct.toFixed(1)}%)</span
              >
            </p>
          </div>
          <div>
            ...
            <p
              class="text-lg font-bold {variance.variance_total >= 0
                ? 'text-green-600'
                : 'text-red-600'}"
            >
              {variance.variance_total >= 0 ? "+" : ""}{formatCurrency(
                variance.variance_total,
              )}
              <span class="text-sm font-normal"
                >({variance.variance_total_pct.toFixed(1)}%)</span
              >
            </p>
          </div>
        </div>
        ...
        <div class="space-y-3">
          <div>
            <div class="flex justify-between text-sm text-gray-600 mb-1">
              <span
                >{$_("budgets.ordinary")}: {formatCurrency(
                  variance.actual_ordinary,
                )} / {formatCurrency(variance.budgeted_ordinary)}</span
              >
              <span
                >{Math.round(
                  (variance.actual_ordinary / variance.budgeted_ordinary) * 100,
                )}%</span
              >
            </div>
            <div class="w-full bg-gray-200 rounded-full h-2">
              <div
                class="h-2 rounded-full {Math.min(
                  100,
                  (variance.actual_ordinary / variance.budgeted_ordinary) * 100,
                ) > 100
                  ? 'bg-red-500'
          ...
          <div>
            <div class="flex justify-between text-sm text-gray-600 mb-1">
              <span
                >{$_("budgets.extraordinary")}: {formatCurrency(
                  variance.actual_extraordinary,
                )} / {formatCurrency(variance.budgeted_extraordinary)}</span
              >
              <span
                >{Math.round(
                  (variance.actual_extraordinary /
                    variance.budgeted_extraordinary) *
                    100,
                )}%</span
              >
            </div>
        ...
        <div class="mt-4 p-3 bg-gray-50 rounded-lg text-sm text-gray-600">
          <p>
            {$_("budgets.monthsElapsed", {
              values: { months: variance.months_elapsed },
            })} | {$_("budgets.yearEndProjection")}: {formatCurrency(
              variance.projected_year_end_total,
            )}
          </p>
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
        ...
        <div class="mb-4">
          ...
          <textarea
            ...
            rows="3"
            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500"
            placeholder={$_("budgets.rejectionReasonPlaceholder")}></textarea>
        </div>
        <div class="flex justify-end space-x-3">
          <button
