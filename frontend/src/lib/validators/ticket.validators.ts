...
import { required, minLength, collectErrors, type ValidationResult } from "./common.validators";
...
export function validateCreateTicket(
  ...
  const errors: Record<string, ValidationResult> = {
    ...
    description: minLength(data.description, 10, messages?.descriptionMinLength),
  };
