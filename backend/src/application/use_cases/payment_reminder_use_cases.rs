use crate::application::dto::{
    ...
    OverdueExpenseDto, PaymentRecoveryStatsDto, PaymentReminderResponseDto,
    ReminderLevelCountDto, ReminderStatusCountDto,
};
use crate::application::ports::{ExpenseRepository, PaymentReminderRepository};
use crate::domain::entities::{PaymentReminder, PaymentStatus, ReminderLevel, ReminderStatus};
...
impl PaymentReminderUseCases {
    ...
    pub async fn create_reminder(
        ...
        let owner_id = Uuid::parse_str(&dto.owner_id)
            .map_err(|_| "Invalid owner_id format".to_string())?;
    ...
    pub async fn bulk_create_reminders(
        ...
        for overdue in overdue_list {
            ...
            if expense_id.is_err() || owner_id.is_err() {
                errors.push(format!(
                    "Invalid UUID format for expense_id or owner_id in overdue item"
                ));
                skipped_count += 1;
                continue;
            }
            ...
            match expense_result {
                Ok(Some(expense)) => {
                    ...
                    match self.create_reminder(create_dto).await {
                        ...
                        Err(e) => {
                            errors.push(format!("Error creating reminder for expense {}: {}", expense_id, e));
                            skipped_count += 1;
                        }
                    }
                }
    ...
    pub async fn process_automatic_escalations(&self) -> Result<i32, String> {
        ...
        for reminder_dto in reminders {
            let id = Uuid::parse_str(&reminder_dto.id)
                .map_err(|_| "Invalid reminder ID".to_string())?;
    ...
    pub async fn recalculate_all_penalties(
        &self,
        organization_id: Uuid,
    ) -> Result<i32, String> {
        let reminders = self
