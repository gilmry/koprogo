...
/// Helper: map a DB row to a Meeting entity (includes quorum fields)
fn row_to_meeting(row: &sqlx::postgres::PgRow) -> Meeting {
    ...
    Meeting {
        ...
        quorum_validated: row.try_get("quorum_validated").unwrap_or(false),
        quorum_percentage: row.try_get("quorum_percentage").unwrap_or(None),
        total_quotas: row.try_get("total_quotas").unwrap_or(None),
...
impl MeetingRepository for PostgresMeetingRepository {
    ...
    async fn find_by_id(&self, id: Uuid) -> Result<Option<Meeting>, String> {
        let query = format!("SELECT {} FROM meetings WHERE id = $1", SELECT_COLUMNS);
    ...
    async fn find_by_building(&self, building_id: Uuid) -> Result<Vec<Meeting>, String> {
        let query = format!(
            "SELECT {} FROM meetings WHERE building_id = $1 ORDER BY scheduled_date DESC",
            SELECT_COLUMNS
        );
    ...
    async fn find_all_paginated(
        ...
        let data_query = format!(
            "SELECT {} FROM meetings {} ORDER BY scheduled_date DESC LIMIT {} OFFSET {}",
            SELECT_COLUMNS,
            where_clause,
            page_request.limit(),
            page_request.offset()
        ...
        Ok((rows.iter().map(row_to_meeting).collect(), total_items))
    }
}
