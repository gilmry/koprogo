...
pub async fn create_reminder(
        ...
        .await
    {
        Ok(reminder) => {
            ...
            AuditLogEntry::new(
                AuditEventType::Other("PaymentReminderCreated".to_string()),
                Some(user.user_id),
...
pub async fn mark_as_sent(
        ...
        .await
    {
        Ok(reminder) => {
            AuditLogEntry::new(
                AuditEventType::Other("PaymentReminderSent".to_string()),
                Some(user.user_id),
                Some(organization_id),
            )
...
pub async fn mark_as_opened(
        ...
        .await
    {
        Ok(reminder) => {
            AuditLogEntry::new(
                AuditEventType::Other("PaymentReminderOpened".to_string()),
                Some(user.user_id),
                Some(organization_id),
            )
            .with_resource("PaymentReminder", *id)
...
pub async fn mark_as_paid(
    ...
    match state.payment_reminder_use_cases.mark_as_paid(*id).await {
        Ok(reminder) => {
            AuditLogEntry::new(
                AuditEventType::Other("PaymentReminderPaid".to_string()),
                Some(user.user_id),
                Some(organization_id),
            )
            .with_resource("PaymentReminder", *id)
...
pub async fn cancel_reminder(
        ...
        .await
    {
        Ok(reminder) => {
            AuditLogEntry::new(
                AuditEventType::Other("PaymentReminderCancelled".to_string()),
                Some(user.user_id),
                Some(organization_id),
            )
            .with_resource("PaymentReminder", *id)
...
pub async fn escalate_reminder(
        ...
        .await
    {
        Ok(Some(reminder)) => {
            AuditLogEntry::new(
                AuditEventType::Other("PaymentReminderEscalated".to_string()),
                Some(user.user_id),
                Some(organization_id),
            )
            .with_resource("PaymentReminder", *id)
...
pub async fn add_tracking_number(
        ...
        .await
    {
        Ok(reminder) => {
            AuditLogEntry::new(
                AuditEventType::Other("PaymentReminderTrackingAdded".to_string()),
                Some(user.user_id),
                Some(organization_id),
            )
            .with_resource("PaymentReminder", *id)
...
pub async fn bulk_create_reminders(
        ...
        .await
    {
        Ok(response) => {
            AuditLogEntry::new(
                AuditEventType::Other("PaymentRemindersBulkCreated".to_string()),
                Some(user.user_id),
                Some(organization_id),
            )
...
pub async fn delete_reminder(
    ...
    match state.payment_reminder_use_cases.delete_reminder(*id).await {
        Ok(true) => {
            AuditLogEntry::new(
                AuditEventType::Other("PaymentReminderDeleted".to_string()),
                Some(user.user_id),
                Some(organization_id),
            )
            .with_resource("PaymentReminder", *id)
