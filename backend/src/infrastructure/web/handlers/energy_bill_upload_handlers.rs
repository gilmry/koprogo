...
use crate::application::use_cases::{EnergyBillUploadUseCases, EnergyCampaignUseCases};
...
pub async fn upload_bill(
    ...
    let encryption_key =
        get_encryption_key().map_err(|e| actix_web::error::ErrorInternalServerError(e))?;

    let upload = EnergyBillUpload::new(
        request.campaign_id,
        request.unit_id,
        user.building_id
            .ok_or_else(|| actix_web::error::ErrorBadRequest("Building ID required"))?,
    ...
    let campaign_use_cases = web::Data::from(std::sync::Arc::clone(&uploads.upload_repo)
        as std::sync::Arc<dyn crate::application::ports::EnergyBillUploadRepository>);
...
pub async fn get_my_uploads(
    ...
    let unit_id = user
        .building_id
        .ok_or_else(|| actix_web::error::ErrorBadRequest("Unit ID required"))?;
