use crate::application::dto::{
    BorrowObjectDto, CreateSharedObjectDto, DeleteSharedObjectDto, UpdateSharedObjectDto,
};
...
pub async fn create_shared_object(
        ...
        .await
    {
        ...
        Err(e) => {
            // Issue #781 — ce refus est un 403 (règle métier : prêter un
            // objet engage une personne, pas la copropriété), jamais un 400.
            // Le `kind` laisse le frontend router vers un message traduit
            // dans les quatre locales sans dépendre du libellé français.
...
pub async fn delete_shared_object(
    ...
    auth: AuthenticatedUser,
    id: web::Path<Uuid>,
    // Corps optionnel : le propriétaire qui supprime sa propre annonce n'a
    // rien à motiver et peut continuer à appeler cette route sans corps JSON.
    // Un corps MALFORMÉ tombe aussi dans `None` (limite connue d'`Option<Json<T>>`,
    ...
    // le propriétaire qui n'en a pas besoin.
    body: Option<web::Json<DeleteSharedObjectDto>>,
) -> impl Responder {
    let org_id = match auth.require_organization() {
        Ok(id) => id,
        Err(e) => {
    ...
    match data
        .shared_object_use_cases
        .delete_shared_object(id.into_inner(), auth.user_id, org_id, &auth.role, reason)
        .await
    {
