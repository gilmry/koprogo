...
pub async fn create_account(
        ...
        .await
    {
        Ok(account) => {
            // Audit log
            AuditLogEntry::new(
                AuditEventType::Custom("AccountCreated".to_string()),
                Some(user.user_id),
    ...
    match accounts_result {
        Ok(accounts) => {
            let dtos: Vec<AccountResponseDto> =
                accounts.iter().map(|a| account_to_dto(a)).collect();
...
pub async fn update_account(
        ...
        .await
    {
        Ok(account) => {
            // Audit log
            AuditLogEntry::new(
                AuditEventType::Custom("AccountUpdated".to_string()),
                Some(user.user_id),
...
pub async fn delete_account(
    ...
    match state.account_use_cases.delete_account(account_id).await {
        Ok(()) => {
            // Audit log
            AuditLogEntry::new(
                AuditEventType::Custom("AccountDeleted".to_string()),
                Some(user.user_id),
                Some(existing_account.organization_id),
            )
...
pub async fn seed_belgian_pcmn(
        ...
        .await
    {
        Ok(count) => {
            // Audit log
            AuditLogEntry::new(
                AuditEventType::Custom("BelgianPCMNSeeded".to_string()),
                Some(user.user_id),
                Some(organization_id),
            )
