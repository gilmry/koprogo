pub mod account_handlers;
pub mod acp_handlers;
pub mod acp_module_handlers;
pub mod admin_gdpr_handlers;
pub mod ag_session_handlers;
pub mod age_request_handlers;
pub mod api_key_handlers;
pub mod auth_handlers;
pub mod board_decision_handlers;
pub mod board_member_handlers;
pub mod budget_handlers;
pub mod building_handlers;
pub mod call_for_funds_handlers;
pub mod cdc_handlers;
pub mod charge_distribution_handlers;
pub mod conformity_response;
pub mod consent_handlers;
pub mod contractor_evaluation_handlers;
pub mod contractor_report_handlers;
pub mod convocation_handlers;
pub mod dashboard_handlers;
pub mod document_handlers;
pub mod energy_bill_upload_handlers;
pub mod energy_campaign_handlers;
pub mod etat_date_handlers;
pub mod expense_handlers;
pub mod financial_report_handlers;
pub mod financial_report_handlers_building;
pub mod gamification_handlers;
pub mod gdpr_art30_handlers;
pub mod gdpr_handlers;
pub mod health;
pub mod individual_member_handlers;
pub mod iot_grid_handlers;
pub mod iot_handlers;
pub mod journal_entry_handlers;
pub mod legal_handlers;
pub mod local_exchange_handlers;
pub mod magic_link_handlers;
pub mod mandate_handlers;
pub mod marketplace_handlers;
pub mod mcp_sse_handlers;
pub mod meeting_handlers;
pub mod metrics;
pub mod notice_handlers;
pub mod notification_handlers;
pub mod organization_handlers;
pub mod owner_contribution_handlers;
pub mod owner_handlers;
pub mod payment_handlers;
pub mod payment_method_handlers;
pub mod payment_reminder_handlers;
pub mod pcn_handlers;
pub mod poll_handlers;
pub mod portfolio_handlers;
pub mod public_handlers;
pub mod quote_handlers;
pub mod resolution_handlers;
pub mod resource_booking_handlers;
pub mod role_assignment_handlers;
pub mod role_delegation_handlers;
pub mod security_incident_handlers;
pub mod seed_handlers;
pub mod shared_object_handlers;
pub mod skill_handlers;
pub mod stats_handlers;
pub mod syndic_response_handlers;
pub mod technical_inspection_handlers;
pub mod technical_spec_handlers;
pub mod ticket_handlers;
pub mod two_factor_handlers;
pub mod unit_handlers;
pub mod unit_owner_handlers;
pub mod user_handlers;
pub mod work_report_handlers;

pub use account_handlers::*;
pub use acp_handlers::*;
pub use acp_module_handlers::*;
pub use admin_gdpr_handlers::*;
pub use ag_session_handlers::*;
pub use age_request_handlers::*;
pub use api_key_handlers::*;
pub use auth_handlers::*;
pub use board_decision_handlers::*;
pub use board_member_handlers::*;
pub use budget_handlers::*;
pub use building_handlers::*;
pub use call_for_funds_handlers::*;
pub use cdc_handlers::*;
pub use charge_distribution_handlers::*;
pub use consent_handlers::*;
pub use contractor_evaluation_handlers::*;
pub use contractor_report_handlers::*;
pub use convocation_handlers::*;
pub use dashboard_handlers::*;
pub use document_handlers::*;
pub use energy_bill_upload_handlers::*;
pub use energy_campaign_handlers::*;
pub use etat_date_handlers::*;
pub use expense_handlers::*;
pub use financial_report_handlers::*;
pub use financial_report_handlers_building::*;
pub use gamification_handlers::*;
pub use gdpr_art30_handlers::*;
pub use gdpr_handlers::*;
pub use health::*;
pub use individual_member_handlers::*;
pub use iot_grid_handlers::*;
pub use iot_handlers::*;
pub use journal_entry_handlers::*;
pub use legal_handlers::*;
pub use local_exchange_handlers::*;
pub use magic_link_handlers::*;
pub use mandate_handlers::*;
pub use marketplace_handlers::*;
pub use mcp_sse_handlers::*;
pub use meeting_handlers::*;
pub use metrics::*;
pub use notice_handlers::*;
pub use notification_handlers::*;
pub use organization_handlers::*;
pub use owner_contribution_handlers::*;
pub use owner_handlers::*;
pub use payment_handlers::*;
pub use payment_method_handlers::*;
pub use payment_reminder_handlers::*;
pub use pcn_handlers::*;
pub use poll_handlers::*;
pub use portfolio_handlers::*;
pub use public_handlers::*;
pub use quote_handlers::*;
pub use resolution_handlers::*;
pub use resource_booking_handlers::*;
pub use role_assignment_handlers::*;
pub use role_delegation_handlers::*;
pub use security_incident_handlers::*;
pub use seed_handlers::*;
pub use shared_object_handlers::*;
pub use skill_handlers::*;
pub use stats_handlers::*;
pub use syndic_response_handlers::*;
pub use technical_inspection_handlers::*;
pub use technical_spec_handlers::*;
pub use ticket_handlers::*;
pub use two_factor_handlers::*;
pub use unit_handlers::*;
pub use unit_owner_handlers::*;
pub use user_handlers::*;
pub use work_report_handlers::*;
