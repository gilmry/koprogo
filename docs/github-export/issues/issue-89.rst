==================================================================
Issue #89: feat: Digital Maintenance Logbook (Carnet d'Entretien)
==================================================================

:State: **OPEN**
:Milestone: Jalon 4: Automation & Intégrations 📅
:Labels: enhancement,phase:k3s track:software,priority:high legal-compliance,maintenance
:Assignees: Unassigned
:Created: 2025-11-01
:Updated: 2025-11-13
:URL: `View on GitHub <https://github.com/gilmry/koprogo/issues/89>`_

Description
===========

.. raw:: html

   <div class="github-issue-body">

::

   # Issue #020 - Carnet d'Entretien Digital
   
   **Priorité**: 🟡 IMPORTANT  
   **Estimation**: 10-12 heures  
   **Phase**: Phase 2 K3s + Automation  
   
   ## 📋 Description
   
   Carnet d'entretien digital avec rapports travaux, inspections techniques, garanties.
   
   **Gap**: 0% implémenté - Obligation légale belge pour suivi maintenance
   
   ## 🎯 Objectifs
   
   - [ ] Entity WorkReport (contractor, date, description, photos, cost)
   - [ ] Entity TechnicalInspection (type, inspector, date, report, next_due)
   - [ ] Gestion garanties: 2 ans (apparents), 10 ans (décennale)
   - [ ] Alertes inspections obligatoires
   - [ ] Upload photos avec métadonnées EXIF
   - [ ] Historique complet interventions
   - [ ] Export PDF carnet pour vente/audit
   
   ## 📐 Inspections Obligatoires
   
   - Ascenseur: annuel
   - Chaudière: annuel
   - Électricité: 5 ans
   - Extincteurs: annuel
   
   ## ✅ Critères d'Acceptation
   
   - Carnet digital complet
   - Alertes inspections automatiques
   - Export PDF conforme notaires
   - Tests E2E workflow maintenance
   
   ---
   
   **Voir**: \`issues/important/020-carnet-entretien.md\`

.. raw:: html

   </div>

