...
### 🚨 TROUVÉ, PAS FIXÉ — "Demander un devis" (`/quotes`) : incohérence d'architecture backend/frontend, fonctionnalité 100% cassée

- **Constat** : `QuoteList.svelte` (`handleCreate()`) envoie `POST /quotes` avec seulement `{building_id, contractor_id, project_title, project_description, work_category}` — c'est la sémantique attendue d'une **demande** de devis (le syndic sollicite un prix, ne le connaît pas encore). Mais le DTO backend `CreateQuoteDto` (`quote_dto.rs:7-18`) exige en plus, **tous non-optionnels** : `amount_excl_vat`, `vat_rate`, `validity_date`, `estimated_duration_days`, `warranty_years` (`estimated_start_date` est le seul champ optionnel). La table `quotes` (migration `20251120150000_create_quotes.sql`) a ces mêmes colonnes en `NOT NULL` au niveau DB. Résultat : **tout** clic sur "Demander un devis" échoue systématiquement en 400 (`Json deserialize error: missing field 'amount_excl_vat'`), reproduit directement en `curl` sur `POST /quotes`.
- **Preuve que c'est bien une incohérence de conception, pas un oubli frontend** : le backend a un endpoint distinct `POST /quotes/{id}/submit` (`quote_handlers.rs:110`) et le frontend a un DTO `SubmitQuoteDto` séparé (`amount_excl_vat_cents`, `vat_rate`, `validity_date`, `estimated_duration_days`, `warranty_years`) dans `lib/api/quotes.ts:57-63` — le statut `QuoteStatus::Requested` (premier statut du workflow, `quote_status` enum) confirme que le modèle de domaine est bien pensé en 2 phases (1. syndic demande, 2. contractant soumet un prix plus tard). C'est `CreateQuoteDto` qui contredit ce modèle en exigeant les champs de la phase 2 dès la phase 1.
- **Pourquoi pas fixé dans ce sweep** : la correction correcte touche soit le schéma DB (rendre les colonnes prix nullable + migration), soit le contrat `CreateQuoteDto` (rendre les champs `Option<>` avec une valeur par défaut cohérente au niveau use case), et implique une décision de conception (est-ce qu'un devis "Requested" doit avoir des valeurs de prix à 0/NULL en attendant la soumission du contractant ?) qui dépasse le cadre d'un fix mécanique de ce sweep. Signalé pour arbitrage @gilmry plutôt que décidé unilatéralement.
- **Sévérité** : haute — fonctionnalité "Demander un devis" syndic entièrement non-fonctionnelle en l'état, pour tous les syndics, sans aucun message d'erreur utilisateur clair (juste un toast générique `errorMessage`).
