...
> - **[#695](https://github.com/gilmry/koprogo/issues/695)** 🟡 **FIX POUSSÉ, CI EN COURS** (2026-08-08, régression **WP-CL6/H15**) : les helpers de seed BDD (`backend/tests/bdd_operations.rs`) n'ont pas suivi le retrait de `units.organization_id` (commit `b9fa9d6`), 137 scénarios sur 592 échouent (`column "organization_id" of relation "units" does not exist` + violations NOT NULL sur `acp_id`). Trouvée en vérifiant la CI de `feature/dev` (run `31267524648`) après #617/#634, non-régressive sur ce push précis. **2026-08-21** : 4 sites d'`INSERT INTO units` restants corrigés (`organization_id`→`acp_id` via `ensure_default_acp_for_org`, pattern déjà utilisé dans le fichier) ; les variantes citées dans l'issue d'origine (poll/etat_date/legal_holds/Alice) n'existaient déjà plus sous ces noms. `rustfmt --check` propre ; compile-check bloqué en session (Docker indisponible + proxy réseau restreint sur le build-script `utoipa-swagger-ui`) → validation reportée à la CI GitHub Actions (PR suivie). Log `docs/agent-activity/2026-08-21-fix-695-bdd-operations-acp-id.md`.
