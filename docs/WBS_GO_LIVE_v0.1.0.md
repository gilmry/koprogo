...
- [ ] Terraform appliqué + état distant (F1/F4) ; rôles Ansible convergés (F1) — **À RECONFIRMER (2026-08-26)** : le service **tourne** (koprogo.com, VPS ecosolva `57.128.85.217`), mais rien ne prouve que ce VPS a été provisionné *par Terraform* avec état distant ni que les rôles Ansible ont convergé. Vérification humaine requise ; ce n'est plus « indisponible », c'est « non vérifié ».
- [x] Cert Let's Encrypt valide sur 443, http→https OK (F2) — **FAIT, vérifié 2026-08-26** : `https://koprogo.com` → 200 avec `ssl_verify_result=0`, `https://api.koprogo.com/api/v1/health` → 200. DNS A des deux noms → `57.128.85.217`, `ACME_EMAIL` renseigné.
