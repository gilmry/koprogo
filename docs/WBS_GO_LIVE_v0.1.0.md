...
>
> **🔴 Blocages go-live tracés (hors Track H, à clore avant tag v0.1.0)** — **resync 2026-08-21** : 6/9 **FAIT**, 3 ouverts (#699, #695, #696).
...
> - **[#695](https://github.com/gilmry/koprogo/issues/695)** 🔴 **OUVERT** (2026-08-08) — régression **WP-CL6/H15** : les helpers de seed BDD (`backend/tests/bdd_operations.rs`) n'ont pas suivi le retrait de `units.organization_id` (commit `b9fa9d6`), 137 scénarios sur 592 échouent (`column "organization_id" of relation "units" does not exist` + violations NOT NULL sur `acp_id`). Trouvée en vérifiant la CI de `feature/dev` (run `31267524648`) après #617/#634, non-régressive sur ce push précis mais jamais tracée dans ce WBS avant le resync du 2026-08-21.
