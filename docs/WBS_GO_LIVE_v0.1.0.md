...
- **WP-H6 — Slice 5 modularité + onboarding + RBAC Communauté Moderator** · Maury stories 5.1-5.8 · T2 · M · _ARBITRAGE v0.1.0 vs v0.2.0_
  Table `acp_enabled_modules` + middleware `ModuleGuard` + `ModuleDisabledError` + UI `ModuleGate.svelte` + Syndic = `community.moderator` (RBAC SEL/Poll/Notice/SharedObject sans participation perso, INV-4 brief) + Reservation.on*behalf_of_acp + Comptable 403 sur /community/* + activation/désactivation auditée + archivage (jamais delete) + onboarding wizard ≤ 5min + CI gate axe-core ≥ 90 + data-testid lint. 8 stories `story/5.1-_`à`story/5.8-\*`. Deps : WP-H1 + WP-H4 (sous-rôles).
...
> - **WP-CL6** ✅ — H15 migration `units.organization_id`→`acp_id` (`b9fa9d6`) : trio réversible (add nullable → backfill `building.acp_id` → NOT NULL + drop org + policy/index) + domaine/DTO/repo runtime SQL/handlers scope_guard #603/seed. Validé testcontainers (e2e_units 6/6, integration_unit_owner 13/13, bdd_building_conformity 9/9, bdd_validate_before_compute). Cohérence post-#602 (comme buildings). **⚠️ Régression BDD non couverte par cette validation testcontainers, cf. #695 ci-dessous.**
>
> **🔴 Blocages go-live tracés (hors Track H, à clore avant tag v0.1.0)** — **2026-08-21 (PR #708 mergée `df9b69e`)** : 8/9 **FAIT** (#695, #699 fermés), 1 ouvert (#696, non-actionnable — cf. note ci-dessous).
>
...
> - **[#605](https://github.com/gilmry/koprogo/issues/605)** ✅ **FAIT** (`e47634e9`) — `loginViaUI()` enchaînait un `goto()` juste après `waitForURL()` sans laisser le cookie de refresh se stabiliser, la nav suivante pouvait déclencher un silent-refresh 401 (~4% des runs). Fix : `waitForLoadState("networkidle")` après login (même pattern que `humanLogin()` de `role-assignment.spec.ts`). Vérifié 30/30 vert (`workers=1 --repeat-each=6`, config CI). Fermé 2026-08-09.
> - **[#699](https://github.com/gilmry/koprogo/issues/699)** ✅ **FAIT** (fermée 2026-08-21, PR #708 `df9b69e`) (2026-08-09) — `npm audit` 3 vulns high : `@babel/plugin-transform-modules-systemjs` (chaîne `@vite-pwa/astro`→`workbox-build`) + `js-yaml`/CVE-2026-59870 (chaîne `@redocly/openapi-core`). Dev-only, aucune des deux dans le bundle prod. Au 2026-08-09, `npm audit fix --dry-run` ne résolvait rien (pas de version non-breaking dispo alors publiée). **2026-08-21** : re-vérifié — des patch releases amont sont désormais disponibles (`@babel/plugin-transform-modules-systemjs` 7.29.0→7.29.8, hors de la plage vulnérable `7.12.0-7.29.0` ; `js-yaml` 4.3.0→4.3.1, hors de `4.0.0-4.3.0`), toutes deux compatibles avec les ranges déjà déclarés par les dépendants (`@babel/preset-env ^7.29.0`, `astro`/`@astrojs/internal-helpers ^4.3.0`). `overrides` npm ciblé ajouté (pattern #634), lockfile régénéré via `npm install --package-lock-only`. Vérifié : `npm audit` → **0 vulnérabilité** ; `npm ci` + `npm run build` (115 pages) + `prettier --check` propres. Diff lockfile scopé aux seules versions bumpées (pas de collatéral). (Doublon **#687** fermé le 2026-08-21, même vuln babel, sans action code.)
> - **[#695](https://github.com/gilmry/koprogo/issues/695)** ✅ **FAIT** (fermée 2026-08-21, PR #708 `df9b69e`, `BDD Tests` vert en CI réelle) (2026-08-08, régression **WP-CL6/H15**) : les helpers de seed BDD n'ont pas suivi le retrait de `units.organization_id` (commit `b9fa9d6`), 137 scénarios sur 592 échouent sur `bdd_operations` (`column "organization_id" of relation "units" does not exist` + violations NOT NULL sur `acp_id`). Trouvée en vérifiant la CI de `feature/dev` (run `31267524648`) après #617/#634, non-régressive sur ce push précis. **2026-08-21** : 4 sites corrigés dans `backend/tests/bdd_operations.rs` (`organization_id`→`acp_id` via `ensure_default_acp_for_org`) — **la 1ʳᵉ passe CI a révélé que le même bug frappait aussi `bdd.rs` (2 sites), `bdd_financial.rs` (6 sites) et `bdd_governance.rs` (6 sites)**, la portée réelle dépassait le seul fichier nommé par l'issue d'origine ; les 14 sites supplémentaires corrigés au même pattern. `grep -rn "INSERT INTO units" backend/tests/*.rs \| grep -v acp_id` → 0 résultat sur les 4 fichiers. `rustfmt --check` propre ; compile-check bloqué en session (Docker indisponible + proxy réseau restreint sur le build-script `utoipa-swagger-ui`) → validation reportée à la CI GitHub Actions (PR suivie). Log `docs/agent-activity/2026-08-21-fix-695-bdd-operations-acp-id.md`.
> - **[#696](https://github.com/gilmry/koprogo/issues/696)** 🔴 **OUVERT, non-actionnable en l'état** (2026-08-08) — instabilité pré-existante de la suite smoke Playwright CI, 109 échecs/108 passés sur le même run `31267524648`, explicitement **non liée** à #617/#634 (les 8 specs `phase-b-fe/` du périmètre #617 passent 12/12). Répartie sur ~22 fichiers de specs plus anciens (`LocalExchanges`, `Resolutions`, `Payments`, `Invoices`, etc.). **Vérifié 2026-08-21** : l'issue elle-même précise qu'elle sert uniquement à dater une occurrence (pas à rouvrir l'investigation) et renvoie le fond du sujet à **#550** (strate 3, déjà tracée séparément) ; hypothèse non confirmée de dégradation Vite dev sous charge soutenue, à observer sur plusieurs runs avant d'agir. Pas de fix « comme ça » possible sans comprendre la cause (règle CRITICAL.md) → laissé en l'état, cf. #550 pour le vrai travail de fond.
>
...
- [x] `make ci` VERT en local avant push ; BDD jugé par-scénario, zéro régression @security/@negative — **FAIT** : PR #708 (#695 élargi + #699 + svelte-check + RUNBOOK) mergée `df9b69e`. Tous les jobs CI verts (`BDD Tests`, `Contract Types Check`, `Unit Tests`, `E2E Tests`, `Lint & Format Check`, `Frontend Unit Tests`, `Build`) sauf `Playwright E2E Tests` — confirmé strictement pré-existant (8/8 échecs identiques au baseline `feature/dev` HEAD `cf7326d` avant tout diff, 1 flaky recovered au retry), cf. #696.
...
- [ ] Plancher Playwright smoke ≈219/240 ; specs skippés un-skippés ou documentés (D1) ; vitest VERT composants critiques (D2) — **PARTIEL** : WP-D1 = _PARTIEL FAIT_ (strate 3 différée, plancher smoke jamais reconfirmé en CI post-merge — cf. aussi l'instabilité #696 non résolue, reste bloquant pour cocher cette ligne). WP-D2 : **combler 2026-08-21** — `Frontend Unit Tests (vitest)` vert en CI réelle, couvre `stores/auth.test.ts` + `components/meetings/QuorumPanel.test.ts` + désormais `components/convocations/ConvocationPanel.test.ts` (4-cat, ajouté cette session, 348/348 vitest verts). Ligne encore non cochée uniquement à cause du volet D1 (Playwright smoke).
...
- [ ] Terraform appliqué + état distant (F1/F4) ; rôles Ansible convergés (F1) — **À RECONFIRMER (2026-08-26)** : le service **tourne** sur `151.80.107.254` (koprogo.com) et sur le VPS ecosolva, mais rien ne prouve que ce VPS a été provisionné *par Terraform* avec état distant ni que les rôles Ansible ont convergé. Vérification humaine requise ; ce n'est plus « indisponible », c'est « non vérifié ».
- [x] Cert Let's Encrypt valide sur 443, http→https OK (F2) — **FAIT, vérifié 2026-08-26** : `https://koprogo.com` → 200 avec `ssl_verify_result=0`, `https://api.koprogo.com/api/v1/health` → 200. DNS A des deux noms → `151.80.107.254`, `ACME_EMAIL` renseigné.
- [ ] `gitops-deploy.sh watch` systemd actif ; drill deploy+rollback + drill restore GPG+S3 faits (F3/F4) — **PARTIEL (2026-08-26)** : un déploiement continu réel existe (cron `/etc/cron.d/ecosolva-auto-deploy` toutes les 5 min sur `feature/dev`, `.deployed_rev_ecosolva` à jour), mais ce n'est pas l'unit systemd `gitops-deploy.sh watch` prévue, et **aucun drill rollback ni restore GPG+S3 n'a été joué**. Les drills restent le vrai reste à faire.
...
- [ ] Rapport revue humaine daté frais — GO signé bêta fermée ; rapport 2026-04-01 archivé non utilisé (G1) — **OUVERT, Tier 1, désormais DÉBLOQUÉ** : le prérequis « système vivant » est rempli (koprogo.com répond). Il ne manque plus qu'un humain qui teste et signe.
- [ ] Tag `v0.1.0` créé par HUMAIN après GO signé (G2) — **OUVERT, Tier 1** : bloqué par G1.

**Bilan 2026-08-21 (final, post-merge PR #708 `df9b69e`)** : 20/26 critères FAIT, 1 partiellement couvert (D1/D2 — D1 reste borné par #696), 1 partiellement livré (RUNBOOK, revue humaine manquante), 1 cascade `String`→`AppError` différée non-bloquante, **5 critères Tier-1 F1/F2/F3/G1/G2 structurellement hors de portée d'une session agent sandboxée** (nécessitent VPS OVH réel, DNS réel, et un humain physiquement présent pour la revue GO/NO-GO et le tag). Le chemin critique restant vers v0.1.0 est désormais **entièrement côté humain** : provisionner le VPS (F1), câbler DNS/TLS (F2), démarrer le poller GitOps + drills (F3), faire la revue humaine fraîche (G1), puis tagger (G2) — plus, en parallèle et non-bloquant DoD, traiter #661 (quorum f64 légal) dans une session avec `cargo`/Docker fonctionnel avant d'accepter définitivement l'amendement ADR-0008.

### Session 2026-08-26 — #661 traité (blocage d'outillage levé), F1/F2/F3 rattrapés par la réalité

**Ce qui a changé depuis la clôture du 2026-08-21**, et qui invalide deux de ses conclusions :

1. **#661 est FAIT.** La clôture du 21/08 ne l'avait pas laissé de côté par arbitrage de périmètre mais par honnêteté d'outillage (« cette session n'a aucune capacité de compilation Rust »). Cet environnement-ci a Docker (via `sudo`) : l'image `Dockerfile.dev` a été reconstruite et **tout a été exécuté pour de vrai**. Les 6 critères de sortie de l'issue sont couverts — quorum `Decimal` de bout en bout, **unification** de l'Art. 3.87 §5 en prédicats partagés sur `Meeting` (le distanciel applique désormais le quorum DOUBLE têtes+quotités, ce qu'il ne faisait pas), `validate_proxy_mandate` supprimé, `BudgetVarianceResponse` tranché en `Decimal`, gate CI `scripts/check-no-f64-money.sh` câblé dans le job `lint`, et `@edge` sur les deux bornes à 50%. Vérifié : `cargo check --tests` propre, **1665 tests unitaires verts / 0 échec**, `cargo fmt --check` propre, `cargo clippy -D warnings` exit 0. Détail : `docs/agent-activity/2026-08-26-issue661-quorum-decimal.md`. **⇒ l'amendement ADR-0008 n'est plus bloqué par une violation de son propre §B.**

2. **F1/F2/F3 sont satisfaits *de facto*, contrairement à ce que dit le bilan ci-dessus.** KoproGo **est en ligne** : `https://koprogo.com` et `https://api.koprogo.com/api/v1/health` répondent 200 avec un certificat valide, sur le VPS `151.80.107.254`, et une seconde instance tourne sur le VPS ecosolva (`57.128.85.217`) déployée en continu par `/etc/cron.d/ecosolva-auto-deploy` (poller toutes les 5 min sur `feature/dev`). Les critères « OUVERT, Tier 1 — nécessite un VPS OVH réel, indisponible » décrivaient l'environnement de la session précédente, pas l'état du produit. **À faire par un humain** : confirmer que ce déploiement vaut bien F1 (Terraform appliqué + état distant) et F3 (drills deploy/rollback et restore GPG+S3 réellement joués) — être en ligne ne prouve ni l'un ni l'autre.

**Ce qui reste réellement ouvert vers le tag v0.1.0** : D1 (plancher Playwright, borné par #696), la revue humaine fraîche G1, le tag G2, et les drills F3. Plus la dette `f64` monétaire hors quorum, désormais **gelée et visible** par le gate CI plutôt qu'invisible (cf. §Trouvailles du journal du 26/08 : `payment_reminder`, `work_report.cost`, `technical_inspection.cost`, `stats_dto`).
