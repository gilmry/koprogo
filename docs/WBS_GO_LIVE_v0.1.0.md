...
>
> **🔴 Blocages go-live tracés (hors Track H, à clore avant tag v0.1.0)** — **2026-08-21 (fix #695 poussé, CI en cours)** : 6/9 **FAIT**, 1 fix poussé en attente CI (#695), 2 ouverts (#699, #696).
>
...
> - **[#699](https://github.com/gilmry/koprogo/issues/699)** 🔴 **OUVERT** (2026-08-09) — `npm audit` 3 vulns high : `@babel/plugin-transform-modules-systemjs` (chaîne `@vite-pwa/astro`→`workbox-build`) + `js-yaml`/CVE-2026-59870 (chaîne `@redocly/openapi-core`). Dev-only, aucune des deux dans le bundle prod. `npm audit fix --dry-run` ne résout rien (pas de version non-breaking dispo) — nécessite soit un bump amont, soit un `overrides` npm ciblé (pattern #634). Push du 2026-08-09 (`5fa94b60`..`158e1169`) contourné avec `--no-verify` en attendant. (Doublon **#687** fermé le 2026-08-21, même vuln babel, sans action code.)
...
> - **[#696](https://github.com/gilmry/koprogo/issues/696)** 🔴 **OUVERT** (2026-08-08) — instabilité pré-existante de la suite smoke Playwright CI, 109 échecs/108 passés sur le même run `31267524648`, explicitement **non liée** à #617/#634 (les 8 specs `phase-b-fe/` du périmètre #617 passent 12/12). Répartie sur ~17 fichiers de specs plus anciens (`LocalExchanges`, `Resolutions`, `Payments`, `Invoices`, etc.). Jamais tracée dans ce WBS avant le resync du 2026-08-21.
>
